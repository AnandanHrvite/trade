...
- **Exit**: **Profit lock** (per-tick) — once peak P&L ≥ `SCALP_PROFIT_LOCK_TRIGGER(₹500)`, exit when open P&L gives back below `SCALP_PROFIT_LOCK_PCT(50)`% of peak (ratchets up: peak ₹1000 → lock ₹500, peak ₹2000 → lock ₹1000); the only intra-tick exit. → **BB re-entry** (candle close): if price closes back inside the band (failed breakout) exit immediately (`SCALP_BB_REENTRY_EXIT`, default on) → **PSAR flip** on candle close handles trend runners → bid-ask spread guard → EOD. No break-even-to-entry snap, no PSAR/prev-candle SL trail, no % spot-trail, no time-stop.
