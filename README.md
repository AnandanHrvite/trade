...
- **Entry pipeline** — the 09:15–09:30 opening range is **frozen** at 09:30 and never recomputed. A trade requires all of the following, in order:
  ...
  - Entry is always a **candle close**, never intrabar. ATR(5m)/ATR(15m) are anchored at the 09:30 OR freeze so the committed breakout candle can never be re-judged by later data; they are seeded from a multi-day preload while the OR and VWAP stay day-scoped.
...
- **Debugging**: `ORB_DEBUG_TRACE=true` prints the **whole entry funnel** to the logs on every 5-min candle close — time window, OR, day sanity, breakout, body, VWAP, confirmation, retest — each PASS/FAIL/SKIP with its numbers and the final decision. The same trace always rides back on the signal as `sig.gates` (on **every** return path, warm-up included), so the skip log records the full funnel rather than only the first blocking reason. Verbose; turn it off again after diagnosing.
- **Tests**: `npm test` runs the EMA9+VWAP suite **and** [tests/orb.regression.js](tests/orb.regression.js) (22 assertions). The ORB suite guards capital-safety invariants (the rupee clamp can never be exceeded, across every qty × stop-width × side), engine invariants (no repaint, **no look-ahead**, close-only entry, correct stop side), that the six deleted config keys provably no longer change behaviour, and that paper/live/backtest stay in parity on the stop, on crash-recovery persistence and on the duplicate-entry guard. `npm run test:orb` runs it alone.
...
- **Expectation, stated honestly.** Run `node scripts/orbValidate.js --from … --to …` for the authoritative figure — it drives the real engine plus the full production exit stack and prices trades in rupees with costs. On the only sample available (39 sessions, Mar–Apr 2026) it reports **9 trades, 33% win rate, net ~₹3,112, PF 1.39**, bootstrap 95% CI on mean-per-trade **[−₹1,334, +₹2,503]**, and **P(true edge ≤ 0) ≈ 39%**. The single best trade is **231% of net — remove it and the result is −₹4,089.** Strip one trade out of nine and the strategy loses money.
- **Nothing here is statistically proven.** ~147 trades ≈ 637 sessions ≈ 2.5 years are needed for 95% confidence / 80% power. Removals are justified structurally (a filter that deletes the entire right tail of a right-tail strategy is wrong regardless of p-value) and by the need to make the strategy measurable at all — with the prior-day filter on it took **1 trade per 39 sessions**. That removal is Fisher-exact **p = 0.152**, i.e. not significant.
- **Strongest untested hypothesis** — on this sample a *narrow* opening range was the whole edge: `OR < 1.5×ATR15` gave 4 trades / 75% win / **+₹10,700**, while `OR ≥ 1.5×ATR15` gave 5 trades / 0% win / **−₹7,587**. Mechanically sensible (a quiet open stores energy). It is n=9, so `ORB_OR_ATR_MAX` stays at 2.5 rather than being tuned to it — but test this first on the long sample.
...
| `ORB_LIVE_DRY_RUN` | `true` | Keep ORB in dry-run (log only) even when the global harness dry-run is off |
...
| `ORB_MAX_WEEKLY_LOSS` / `ORB_LOSS_STREAK_SKIP` | `9000` / `4` | Weekly realised-loss stop / consecutive losing days before sitting out |
