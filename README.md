...
- **Exit** (per-tick, **spot points**): **Hard stop** — exit if the trade moves ≥ `SCALP_STOP_LOSS_PTS(20)` against entry (symmetric loss cap; the shown PSAR SL is display/sizing only). → **Trailing stop** — once peak favourable spot move ≥ `SCALP_TRAIL_ARM_PTS(15)`, trail `SCALP_TRAIL_GIVEBACK_PTS(15)` behind the best (floor = peak − giveback, ratchets up) and exit on retrace; rides winners, exits on reversal. Points-based so both work even on spot-proxy sessions. → **BB re-entry** (candle close): if price closes back inside the band (failed breakout) exit immediately (`SCALP_BB_REENTRY_EXIT`, default on) → **PSAR flip** on candle close handles trend runners → bid-ask spread guard → EOD. No break-even-to-entry snap, no PSAR/prev-candle SL trail, no % spot-trail, no time-stop.
...
| `SCALP_STOP_LOSS_PTS` | `20` | Hard loss cap — exit if the trade moves this many spot points against entry. Points-based. `0` disables. |
| `SCALP_TRAIL_ARM_PTS` | `15` | Start trailing once the favourable spot move (points) reaches this. `0` disables trailing. |
| `SCALP_TRAIL_GIVEBACK_PTS` | `15` | Once armed, exit when price retraces this many points from the best (floor = peak − giveback, ratchets up) |
