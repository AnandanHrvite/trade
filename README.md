...
### Strategy 1: Swing — EMA 20/50 + RSI + SuperTrend (entry redefined 2026-05-31; PSAR stripped 2026-06-12; EMA9/triple-stack removed 2026-06-19; 3 / 5 / 15-min via env)
- **Entry (intra-candle, all 3 true)**:
  - **CE**: EMA alignment bullish — EMA20 **above** EMA50 (`SWING_EMA_FAST`/`SWING_EMA_SLOW`) · RSI(14) `> RSI_CE_MIN` and `< RSI_CE_MAX` (overbought guard) · **SuperTrend bullish**.
  - **PE**: mirror — EMA20 **below** EMA50 · RSI `< RSI_PE_MAX` and `> RSI_PE_MIN` (oversold guard) · **SuperTrend bearish**.
  - _EMA9 is still computed for the chart/trade-record but is no longer an entry input (the 9>20>50 triple-stack gate whipsawed in chop and was removed 2026-06-19)._
...
| `SWING_EMA_FAST` | `20` | Fast EMA period (close). CE needs EMA-fast above EMA-slow; PE below. The EMA-fast vs EMA-slow cross is the sole directional entry gate. |
| `SWING_EMA_SLOW` | `50` | Slow EMA period (close). The EMA-fast vs EMA-slow alignment is the directional entry gate. |
| `SWING_EMA_FASTEST` | `9` | EMA9 period (close) — computed for the chart/trade-record snapshot only. **Not an entry input** (the EMA9/triple-stack gate was removed 2026-06-19). |
