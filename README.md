...
- **Only sessions that actually ran are replayable.** A recorded day with no session marker for a strategy (it wasn't started, or didn't exist yet) cannot be replayed for it. The recorder only saves the option LTPs a *running* strategy polled, so such a run has no premiums for that strategy's strikes and prices every trade off the spot move 1:1 — a P&L that looks real but is roughly double what the option would actually have done. Those days stay greyed out in the From/To calendars.
