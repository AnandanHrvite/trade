...
- **Exit** (per-tick, **spot points**): **Profit lock** — once peak favourable spot move ≥ `SCALP_PROFIT_LOCK_TRIGGER_PTS(25)`, exit when it gives back below `SCALP_PROFIT_LOCK_PCT(50)`% of peak (ratchets up: peak 100pts → lock 50pts); the upside exit. → **Hard stop** — exit if the trade moves ≥ `SCALP_STOP_LOSS_PTS(30)` against entry; a **wide** catastrophic loss cap that only clips deep adverse excursions on failed fades (the shown PSAR SL is display/sizing only). Both points-based so they work even on spot-proxy sessions. → **BB re-entry** (candle close): if price closes back inside the band (failed breakout) exit immediately (`SCALP_BB_REENTRY_EXIT`, default on) → **PSAR flip** on candle close handles trend runners → bid-ask spread guard → EOD. No break-even-to-entry snap, no PSAR/prev-candle SL trail, no % spot-trail, no time-stop. **Optional entry filter** — **S/R** (`SCALP_SR_FILTER_ENABLED`, off by default): skip a breakout that opens within `SCALP_SR_ZONE_PTS` of a recent swing-high (CE) / swing-low (PE) over the last `SCALP_SR_LOOKBACK` candles.
...
**Entry redefined 2026-05-31.** Entry (intra-candle, all 3 true): **CE** = EMA20 above EMA50 (`SWING_EMA_FAST`/`SWING_EMA_SLOW`, close), RSI(14) `> RSI_CE_MIN` and `< RSI_CE_MAX`, trend source GREEN (SAR below price, or SuperTrend bullish when `SWING_USE_SUPERTREND=true`). **PE** = mirror (EMA20 below EMA50, RSI `< RSI_PE_MAX` and `> RSI_PE_MIN`, trend source RED). **Stop (unchanged)** = previous candle low (CE) / high (PE), trailed candle-by-candle (tighten-only). **Exits**: prev-candle SL · breakeven (`BREAKEVEN_PTS` → SL to entry) · option stop (`OPT_STOP_PCT`) · opposite signal · exit-before-close (`SWING_EOD_EXIT_TIME`) · EOD auto-stop. Same-side cooldown after an SL hit (`SWING_SL_PAUSE_CANDLES`). `EMA21(OHLC4)` is retained only for the `ema` SL mode + the trade-record snapshot, not for entry. **Optional entry filter** — **S/R** (`SWING_SR_FILTER_ENABLED`, off by default): skip a CE into overhead resistance / a PE into support just below (a swing-high/low within `SWING_SR_ZONE_PTS` over the last `SWING_SR_LOOKBACK` candles).
...
| `SWING_SR_FILTER_ENABLED` | `false` | **S/R entry filter** (off by default): skip a CE that opens into overhead resistance / a PE into support just below (a recent swing-high/low within the zone) |
| `SWING_SR_LOOKBACK` | `30` | Candles scanned back for swing-high/low S/R levels |
| `SWING_SR_ZONE_PTS` | `15` | Block when entry price is within this many spot points of a swing level |
...
| `SCALP_BB_REENTRY_EXIT` | `true` | Exit on candle close if price closes back inside the Bollinger Band (failed breakout) |
| `SCALP_SR_FILTER_ENABLED` | `false` | **S/R entry filter** (off by default): skip a CE breakout into overhead resistance / a PE into support just below (a recent swing-high/low within the zone) — targets the failed-breakout fades that bleed on chop days |
| `SCALP_SR_LOOKBACK` | `30` | Candles scanned back for swing-high/low S/R levels |
| `SCALP_SR_ZONE_PTS` | `15` | Block when entry price is within this many spot points of a swing level |
