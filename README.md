...
### Strategy 7: TREND DAY SCALP — a 10:15 day gate, then one bought dip (single-leg slightly-ITM CE/PE, Fyers)
...
### Strategy 8: HA SCALP — a no-wick Heikin Ashi candle in the direction of the 50 MA (single-leg slightly-ITM CE/PE, Zerodha)
...
- **Stop**: the **signal candle's own RAW low (CE) / RAW high (PE)**, then **breakeven + trail** (`HA_SCALP_TRAIL_ENABLED`, default on): the stop lifts to the entry once `HA_SCALP_BREAKEVEN_PTS=15` ahead, then follows the best spot `HA_SCALP_TRAIL_PTS=20` behind from `HA_SCALP_TRAIL_START_PTS=25` ahead. It only ever tightens. Decisions read the Heikin Ashi candle; **prices read the raw candle**, because an HA price is an average of four numbers and nothing ever traded there.
...
- **Deliberately absent**: no auto-drawn trend line (the 50 MA is the sole trend test — a drawn line depends on which swing points you pick and is not reproducible), no VIX/OI/ADX/RSI/ATR/volume filter, no target, no partials.
...
### Strategy 11: EARLYBIRD — the day's first 15-min candle, NIFTY-confirmed, traded in CASH EQUITY (F&O stocks, Fyers)

**Never traded.** Zero paper sessions, zero live orders. Every threshold below is the rule as specified or a round number, not a fitted value.

**`EARLYBIRD_TRADE_MODE` decides what it actually buys** — Settings → "What To Trade":

- **`stock`** (default) — the only strategy here that trades **cash equity**: NIFTY's candle sets the side, a stock must confirm with the same shape, and we trade THAT STOCK, INTRADAY product, a flat share count per name, up to several positions at once. NIFTY is never traded; it only decides whether today is a buy day or a sell day. A PE-side signal is a real intraday **short sale**.
- **`option`** — **NIFTY only.** If NIFTY's own opening candle is a signal candle we buy ONE NIFTY CE/PE off NIFTY's own level, and **no stock is scanned, confirmed or traded** — the confirmation rule is a stock-selection rule, and here no stock is being selected. The ~220-symbol scan does not run at all in this mode.
- **`both`** — both legs at once, independently: the option leg fires even when zero stocks confirm.

Both legs share the same level maths (buffer, big-candle body-edge stop, 1:2 target). **Every level in both legs is a NIFTY/stock SPOT level**, never a premium level — premium moves with IV and theta, so a premium-based stop could not be reproduced in replay. The option leg's P&L is on the premium, so a bought PE profits when spot falls.

- **The signal candle is the day's FIRST 15-minute bar** (09:15→09:30), for NIFTY and for every stock. One bar per symbol per day, never re-evaluated: the decision is made at 09:30 and the rest of the day only executes it.
- **NIFTY must print a one-directional candle.** The three drawings in the source material — a full body, a long favourable wick, a small opposing wick — collapse to a single test: the wick *against* the move ≤ `EARLYBIRD_MAX_OPPOSING_WICK_PCT=30`% of the candle's range, and the body ≥ `EARLYBIRD_MIN_BODY_PCT=40`%. Both are shares of the range, so the tolerance means the same thing on a 30-point index bar and a ₹900 stock bar. Green ⇒ **BUY day**, red ⇒ **SHORT day**.
- **At least `EARLYBIRD_MIN_CONFIRMING_STOCKS=1` stock must agree**: its own first candle passes the same shape test in the same direction. The whole `EARLYBIRD_UNIVERSE=FNO` list (~220 names) is scanned once at 09:30.
- **The 2% gap rule** (stock leg only): a stock opening more than `EARLYBIRD_MAX_GAP_PCT=2`% from its previous daily close is dropped. An **unknown** previous close is a refusal, not a pass — "we don't know the gap" is not "there is no gap". It does not apply to the option leg, which selects no stock.
- **Entry / stop / target**, all frozen at 09:30 and never moved: entry is a pending stop order `EARLYBIRD_ENTRY_BUFFER_PTS=5` beyond the stock's own candle extreme, the stop sits the same distance beyond the other extreme, and the target is `EARLYBIRD_TARGET_RR=2`× the **actual** risk. Set it to `1` for the 1:1 the rules also allow.
- **The big-candle rule**: when wick-to-wick risk exceeds `EARLYBIRD_MAX_SL_PTS=60`, the stop moves off the wick and onto the candle's **body edge** (open/close). It can only ever *tighten*, because the body is always inside the wick, and the 1:2 target is recomputed against the risk actually taken.
- **Time**: no new entries after `EARLYBIRD_ENTRY_END=10:45`; a pending order untouched by then is cancelled. Everything still open is squared off at `EARLYBIRD_FORCED_EXIT=13:00`.
- **Sizing**: stock leg `EARLYBIRD_QTY=100` **shares** (not lots) per name, at most `EARLYBIRD_MAX_CONCURRENT=5` at once; option leg `EARLYBIRD_OPTION_LOTS=1` lot at `EARLYBIRD_ITM_STEPS=1` (~delta 0.6). When more names confirm than the cap allows, the **tightest-stop** ones are taken. Note 100 shares of a ₹3,000 stock is a ₹300,000 position — size both keys deliberately.
- **Deliberately absent**: no VIX gate, no OI filter, no ADX/RSI/EMA/VWAP/SuperTrend, no volume filter, no confirmation candle, no trailing stop, no breakeven jump, no partial booking, no re-entry. `EARLYBIRD_ITM_STEPS` (default 1, ~delta 0.6) applies to the **option leg only**; the stock leg has no strike to pick.
- **How the data arrives.** The 09:30 scan pulls each symbol's intraday and daily series (the daily one is the `prevClose` source for the gap rule), paced by a **two-window rate limiter** (`EARLYBIRD_SCAN_RPS=8` / `EARLYBIRD_SCAN_RPM=180`, concurrency `EARLYBIRD_SCAN_CONCURRENCY=4`). That pacing is *correctness*, not politeness: Fyers answers a rate-limited request the same way it answers a delisted one — with no data — so an over-fast scan silently reports most of the universe as having no candle and the day looks legitimately signal-free. A full ~220-name scan takes about two minutes, well inside the entry window. Afterwards only the **shortlist** is priced, via a `fyers.getQuotes` poll every `EARLYBIRD_POLL_MS=2000`; a quote older than `EARLYBIRD_QUOTE_STALE_SEC=30` stops being used for exits, but the forced-exit clock still fires on it rather than freezing a position.
- **Why a quote poll and not the shared socket**: `socketManager.subscribeExtra()` is built for a handful of NIFTY option contracts and permanently disables extras for the whole session after a run of unattributable ticks. Pushing equity symbols through it would risk taking the shared feed's option multiplexing down for every other strategy.
- **Charges are computed locally.** [src/utils/charges.js](src/utils/charges.js) has only an options and a futures path; billing option STT (0.15% of *premium*) against ₹-lakh equity turnover overstates cost by roughly an order of magnitude. Both Paper and Backtest therefore apply the **equity-intraday** schedule from `EARLYBIRD_CHG_*` / `EARLYBIRD_*_PCT` keys and say so on screen.
- **LIVE = PAPER** (`/early-bird-live`, [src/routes/earlyBirdLiveHarness.js](src/routes/earlyBirdLiveHarness.js)): wraps the Paper engine with the shared harness. **Triple-gated to dry-run**: real orders require `EARLYBIRD_LIVE_ENABLED=true` AND `LIVE_HARNESS_DRY_RUN=false` AND `EARLYBIRD_LIVE_DRY_RUN` not-true. Orders **and** data both go to Fyers, so one login covers both. Open positions are crash-recovered via `positionPersist` (`.active_early_bird_position.json`), which uniquely stores an **array** of positions plus the still-pending setups — losing those on a restart would silently cancel the day's remaining orders.
- **Backtest** (`/early-bird-backtest`, [src/routes/earlyBirdBacktest.js](src/routes/earlyBirdBacktest.js)): drives the **same** engine and re-implements only paper's exits. **Conservative intra-bar ordering**: the stop is tested before the target, so a bar touching both books the LOSS; a bar that opened beyond the stop fills at the **open**. `EARLYBIRD_BT_SLIPPAGE_PTS=0.05` is charged each way (a floor, not a claim). It reports the count of setups that **never triggered**, which for a breakout strategy is a first-class statistic.
- **What the real data says so far.** Across 2,118 cached NIFTY sessions the opening candle qualifies on **~47%** of days (21% buy, 26% short); the other 53% are refused, overwhelmingly for a body below 40% of range. Walking the full lifecycle with **NIFTY standing in for a stock** (there are no cached per-stock candles — this is a proxy, not the strategy's real P&L): 81% of setups trigger, and of the closed trades only **6.6% reach the 1:2 target** while **63% end at the 13:00 bell**. That is the honest headline — the 1pm exit truncates most trades before the target. `EARLYBIRD_FORCED_EXIT` is the lever if you ever want to test holding longer; the rule as specified is 1pm.
- **Guide**: [documents/EarlyBird_Strategy_Guide.html](documents/EarlyBird_Strategy_Guide.html).
...
| URL | Description |
|-----|-------------|
| `POST /settings/reset-paper` | Wipe **all** paper traded data, keep settings: every `/<x>-paper` router with a GET `/reset` is discovered from the Express router stack and its own `/reset` is run in-process (capital from `.env`, sessions + in-memory state cleared; running engines skipped), then every per-day paper trade + skip JSONL is deleted. `GET /settings/reset-paper/targets` lists what would be hit. `API_SECRET`-gated. |
