...
- **Still-outside-band guard** (`SCALP_CONFIRM_OUTSIDE_BAND`, default on; needs confirmation candle on): the confirmation cross only enters while price is **still beyond the live Bollinger band**, not just beyond the signal candle's close. A sharp signal candle can whip the 20-period band wide enough to wrap around the next (confirmation) candle, which makes the entry sit visibly *inside* the band even though the breakout candle had closed outside — this guard blocks those.
