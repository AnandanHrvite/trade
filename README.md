...
| `UI_SHOW_CONSOLIDATION_REPORT` | `true` | Top-level "Consolidation Report" menu (`/consolidation-report`) |
...
| `/consolidation-report` | **Consolidation Report** — one printable, filterable consolidated report of every recorded trade. Book toggle (Paper / Live / **Both**) + per-strategy + a Range preset covering **This week / Last week / This month / Last month / Last 7·30 days / This FY / All time / Custom (from–to)**, plus a **Group by** (Day / Week / Month / Strategy) driving the period-breakdown table. Renders headline cards, an equity curve, a **By Strategy** table, a **By period** table (with running cumulative), and a full **Trade Ledger**. **🖨 Save as PDF** button prints the report through a dedicated `@media print` stylesheet (app chrome hidden, white page, page-break-safe tables) — the browser's native print-to-PDF; no external library. Reads the same session files as `/consolidation` + `/live-consolidation`; writes nothing. Gated by `UI_SHOW_CONSOLIDATION_REPORT`. |
