...
| `RSI_PIVOT_ST_PREMIUM_SL_SIDES` | `BOTH` | Which sides carry the premium floor: `BOTH` / `CE` / `PE` / `NONE`. ⚠️ **PE has no other stop** (it never uses the SuperTrend), so `CE` or `NONE` leaves every PE trade **completely unstopped** — `RSI_PIVOT_ST_EXIT_TIME` becomes its only exit. Same for CE if `RSI_PIVOT_ST_ST_CE_ENABLED` is also off. The engine still takes the trade and warns loudly in the log |
