...
- **Exit**: **Profit lock** (per-tick, **spot points**) — once peak favourable spot move ≥ `SCALP_PROFIT_LOCK_TRIGGER_PTS(25)`, exit when it gives back below `SCALP_PROFIT_LOCK_PCT(50)`% of peak (ratchets up: peak 100pts → lock 50pts, peak 200pts → lock 100pts); the only intra-tick exit. Points-based so it works even on spot-proxy sessions. → **BB re-entry** (candle close): if price closes back inside the band (failed breakout) exit immediately (`SCALP_BB_REENTRY_EXIT`, default on) → **PSAR flip** on candle close handles trend runners → bid-ask spread guard → EOD. No break-even-to-entry snap, no PSAR/prev-candle SL trail, no % spot-trail, no time-stop.
...
| `SCALP_PROFIT_LOCK_PCT` | `50` | Once armed, exit when the favourable move falls below this % of peak (ratchets up) — the only intra-tick exit |
