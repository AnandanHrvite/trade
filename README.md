...
| `EMA_RSI_ST_EMA_EXIT_MODE` | `touch` | How the EMA21 line ends a trade. `touch` (legacy) = the candle only has to **reach** EMA21 (`low ≤ EMA21 ≤ high`) to exit, and EMA21 is also the trailed stop enforced tick-by-tick — so a single wick closes the trade. `close` = **cross & close**: wicks through the line are held and the exit fires only when a candle **closes** beyond EMA21 (CE on a close below it, PE on a close above), logged as `EMA close-through exit`. In `close` mode EMA21 is deliberately **not** used as the trailed stop (a stop parked on EMA21 is hit by the first wick, which would make the two modes identical) — the hard stop is then the candle trail (`EMA_RSI_ST_CANDLE_TRAIL_ENABLED`, keep it on), the initial prev-candle SL, `EMA_RSI_ST_STOP_LOSS_PTS` and `OPT_STOP_PCT`. Identical in paper, live and backtest. |
...
| `BB_RSI_RESOLUTION` | `global` | **BB_RSI-only candle size** — `3`, `5`, or `global` (follow `TRADE_RESOLUTION`). Lets this strategy run on 3-min while every other strategy stays on the global value. Applies to Live, Paper and Backtest together (resolved once in `src/strategies/bb_rsi.js`, so the three modes can't drift). Only 3 and 5 are offered — the band/RSI defaults are tuned for intraday bars, and 15-min leaves a mean-reversion engine too few bars inside the 09:21–14:30 window. Anything else (a typo, a stale `15`) falls back to `TRADE_RESOLUTION`. Paper picks it up on the next session start and Backtest on the next run; **BB_RSI Live caches it at module load, so a change there needs a full server restart** (the same has always been true of `TRADE_RESOLUTION`). **The unified `/all-backtest` dashboard still runs every panel at 5-min** — use `/bb_rsi-backtest` to backtest at 3-min |
| `BB_RSI_DIRECTION` | `fade` | **Which way the signal bars are traded** — `fade` (mean reversion, V8) or `breakout` (momentum, the V7 side mapping). The trigger bars are identical either way; only the side flips, so the two are directly comparable over one backtest range. `breakout` additionally **ignores** `BB_RSI_TARGET_MIDDLE_BAND` — a breakout entry is already past the mean, so the target would fire on the entry bar — leaving the opposite-candle stop/trail, the hard stop and EOD as the exits. Keep `BB_RSI_DIVERGENCE_ENABLED` and `BB_RSI_ADX_ENABLED` **off** in `breakout` mode (both are exhaustion/mean-reversion gates), and note `BB_RSI_MAX_ENTRY_SL_PTS` blocks far more entries there because the stop line is the signal candle's far end. Unrecognised values fall back to `fade`, so an untouched `.env` is unaffected |
| `BB_RSI_BB_PERIOD` / `BB_RSI_BB_STDDEV` | `30` / `2` | Bollinger inputs (charting-standard 30 / 2) |
| `BB_RSI_RSI_CE_THRESHOLD` | `25` | The **oversold / lower-band** trigger — RSI must be at or **below** this. `fade` buys **CE** on it, `breakout` buys **PE**. Note the direction flipped in V8; a V7 value of 70 here will never trade |
| `BB_RSI_RSI_PE_THRESHOLD` | `75` | The **overbought / upper-band** trigger — RSI must be at or **above** this. `fade` buys **PE** on it, `breakout` buys **CE**. Direction flipped in V8 |
| `BB_RSI_RSI_TURNING` | `false` | Also require RSI to have already turned back (CE: RSI rising vs the prior bar; PE: falling) |
| `BB_RSI_CONFIRM_CANDLE_ENABLED` | `true` | **Confirmation candle (cross & close).** `true` (default): a fully-closed candle must meet all entry rules (the *signal candle*), then the **immediately-next** candle must cross that signal candle's close (CE above / PE below — the direction of the expected reversal). Whether that cross is judged intra-bar or at the close is set by `BB_RSI_CONFIRM_ON_CLOSE` below. `false`: enter at the signal candle's close. Filters the one-candle false reversal. A/B via `/replay`. |
| `BB_RSI_CONFIRM_ON_CLOSE` | `true` | **Confirm on candle close** (needs `BB_RSI_CONFIRM_CANDLE_ENABLED=true`). `true` (default): the confirmation candle must **CLOSE** past the signal candle's close — entry fires at that close. `false`: enter intra-bar on the first cross. For a fade, `true` is safer: a poke up from an oversold extreme that closes back down is a falling knife, not a reversal. Replaces the retired `BB_RSI_CONFIRM_OUTSIDE_BAND`. A/B via `/replay`. |
| `BB_RSI_BAND_WIDTH_ENABLED` | `true` | **Chop guard** — skip entries when the band is too narrow to be a real stretch |
| `BB_RSI_MIN_BAND_WIDTH_PTS` | `50` | Upper minus lower band must be at least this many NIFTY points |
| `BB_RSI_RSI_RANGE_ENABLED` | `true` | **Chop guard** — skip entries when RSI has been pinned mid-range (dead tape) |
| `BB_RSI_RSI_RANGE_LOOKBACK` | `20` | Candles **before the signal bar** to measure RSI travel over (the signal bar is excluded so its own spike cannot satisfy the test) |
| `BB_RSI_RSI_RANGE_MIN` | `30` | Max minus min RSI over that lookback must be at least this |
| `BB_RSI_DIVERGENCE_ENABLED` | `false` | Require price/RSI divergence: CE = lower low on a higher RSI low, PE = higher high on a lower RSI high. No pivot in range = blocked (fails closed) |
| `BB_RSI_DIV_LOOKBACK` | `20` | How far back to hunt for the prior swing pivot |
| `BB_RSI_DIV_PIVOT_BARS` | `2` | Bars either side needed to confirm a pivot (higher = stricter, slower) |
| `BB_RSI_MAX_ENTRY_SL_PTS` | `50` | Skip entries whose signal candle sits more than this many pts from its close (avoids uncapped risk). `0` = off |
| `BB_RSI_ADX_ENABLED` | `false` | Trend **ceiling** — block all entries when ADX(14) reaches the max (too strong a trend to fade). |
| `BB_RSI_ADX_MAX` | `30` | Maximum ADX(14) that still allows entries when the ceiling is on (higher = more permissive). **Replaces `BB_RSI_ADX_MIN` — the gate inverted in V8.** |
| `BB_RSI_PROFIT_LOCK_TRIGGER_PTS` | `0` | Optional extra upside cap — arm the profit lock once the favourable spot move (points) hits this. **Off by default in V8**: it fires before the mean is reached and cuts the trade short of its objective. |
...
| `BB_RSI_STOP_LOSS_PTS` | `30` | Per-tick hard stop — exit if the trade moves this many spot points against entry. A real backstop: the two-opposite-candle stop can only fire on a candle **close**, and one violent bar travels a long way first. `0` disables. |
| `BB_RSI_TARGET_MIDDLE_BAND` | `true` | **Take profit at the mean** — per-tick exit when price reaches the BB middle band. The objective of a mean-reversion trade |
| `BB_RSI_OPP_CANDLE_SL_ENABLED` | `true` | **Two-opposite-candle stop** — exit after N consecutive candles whose *body* closed against the trade (CE hurt by red, PE by green; a doji breaks the streak). Arms the per-side cooldown |
| `BB_RSI_OPP_CANDLE_SL_COUNT` | `2` | How many consecutive opposite candles trigger that stop |
| `BB_RSI_OPP_CANDLE_TRAIL_ENABLED` | `true` | **Two-opposite-candle trail** — the same rule, taking over once the trade is in profit. Independent of the stop toggle; does **not** pause the side |
| `BB_RSI_OPP_CANDLE_TRAIL_COUNT` | `2` | How many consecutive opposite candles trigger the trail exit |
| `BB_RSI_TRAIL_ARM_PTS` | `10` | Favourable spot points before the trail takes over from the stop (`0` = immediately) |
| `BB_RSI_SLIPPAGE_PTS` | `0` | Simulated slippage on entry & exit (pts against you) |
...
| `ORB_ENTRY_START` | `09:30` (= `ORB_RANGE_END`) | Earliest breakout candle the hunt will look at — the mirror of `ORB_ENTRY_END`. Defaults to the opening-range freeze, so out of the box the window is unchanged. Added to test "skip the 09:35 entries"; **the evidence points the other way** — on the Jul–Aug 2026 export entries before 09:50 net −₹2,372 over 9 trades while entries after net −₹4,213 over 8, and a delay deletes both of the sample's best trades. Sweep it before trusting it |
...
| `ORB_SL_SOURCE` | `breakout` | Which candle's extreme anchors the initial stop: `entry` (the candle bought on), `breakout` (the first candle that closed past the edge) or `lookback` (the extreme of the last `ORB_SL_LOOKBACK_CANDLES` candles ending at the entry candle) |
| `ORB_SL_LOOKBACK_CANDLES` | `2` | Only read when `ORB_SL_SOURCE=lookback`. `2` = the entry candle **and the one before it**, so the stop clears the recent swing rather than one bar. The window never crosses into the previous session. On a normal ORB entry (confirmation candle immediately after the breakout candle) `lookback` with `2` and `breakout` land on the same level — they only differ when the entry candle's own extreme is the wider one |
...
| `ORB_CANDLE_TRAIL_ENABLED` | `false` | Candle trail: on every candle that **closes in profit**, ratchet the hard SL to the extreme of the last `ORB_CANDLE_TRAIL_CANDLES` candles. Tighten-only — a wider pullback candle never loosens it. Runs alongside the EMA trail (whichever ends the trade first wins) and exits **intrabar**, via the shared hard-SL check, where the EMA trail needs a close. **A hypothesis, not a finding** — sweep it before trusting it |
| `ORB_CANDLE_TRAIL_CANDLES` | `2` | How many candles the trailing stop must clear. `1` = just the candle that closed (tightest); `2` matches the `lookback` initial stop |
...
| **— risk / regime —** | | |
| `ORB_MAX_DAILY_TRADES` | `1` | Textbook 1/day — raise only if you accept the chop. **On its own this can never produce a second entry**: the engine keeps re-finding the day's one committed breakout candle and refuses to enter late. Use `ORB_REENTRY_AFTER_SL` for a real second attempt |
| `ORB_REENTRY_AFTER_SL` | `0` (off) | Extra attempts allowed after the breakout is **stopped out**, on top of `ORB_MAX_DAILY_TRADES`. The breakout hunt **re-arms**: every candle up to and including the stop-out candle becomes invisible, so only a genuinely fresh close beyond the same frozen range edge can re-enter, with the ordinary confirmation/retest rules. Only a stop-out re-arms (hard SL, rupee cap, premium stop) — an EMA-trail, opposite-candle or EOD exit means the *move* ended, not that the breakout was wrong. Owner cases: 2026-08-03 (stopped 10:35 on a 24pt stop, day traded higher into 12:35) would re-enter ~10:45; 2026-07-29 (stopped 09:55) at ~10:05. Re-entries are tagged `[re-entry]` in the entry reason. **Unproven — sweep it** |
...
| `RSI_PIVOT_ST_ST_SIDES` | `CE` | Which sides carry the SuperTrend stop and trail: `BOTH` / `CE` (the original rule) / `PE` / `NONE`. On PE the line is **mirrored** — it sits above price and a flip to *bullish* is the exit. A side that carries it also gains the "SuperTrend must be on the right side of price" entry check, so it takes **fewer** trades. Legacy `RSI_PIVOT_ST_ST_CE_ENABLED=false` still means `NONE` when this key is unset |
| `RSI_PIVOT_ST_PREMIUM_SL_PCT` | `25` | Exit when the option price falls this far below its **high-water** price. Bought at 100 → exit at 75; runs to 140 → the exit rises to 105 |
| `RSI_PIVOT_ST_PREMIUM_SL_SIDES` | `BOTH` | Which sides carry the premium floor: `BOTH` / `CE` / `PE` / `NONE`. ⚠️ A side with **no premium floor AND no SuperTrend** (see `RSI_PIVOT_ST_ST_SIDES`) is **completely unstopped** — `RSI_PIVOT_ST_EXIT_TIME` becomes its only exit. At the defaults that is the PE side, since SuperTrend defaults to `CE`. The engine still takes the trade and warns loudly in the log |
...
| `MANUAL_TRADES_AUTO_SYNC_ENABLED` | `true` | Daily 15:35 IST auto-pull of today's manual **Kite** fills (via `zerodhaBroker.getTrades()`) into `/pnl-history` → Manual Trading Analytics, same self-rescheduling-timer pattern as the EOD reporter (`manualTradesSyncJob.js`). Kite Connect has no historical order/trade API, so this only ever captures **today going forward** — past manual trades need a one-time Kite Console → Reports → Tradebook CSV import (the **Import Tradebook CSV** button on the same page). Off = the page still has a manual **Sync Now** button and CSV import; only the unattended daily pull is disabled. |
...
| `UI_SHOW_SWING_SCANNER` | `false` | Top-level "Swing Scanner" menu (`/swing-scanner`) — stock screen + manual positional orders. Default **off**: it is the only page in this app that can place a real broker order with **no dry-run gate**, so it appears because you switched it on. |
...
| `UI_SHOW_TOKEN_SYNC` | `true` | Show **Token Sync** in the System sidebar group (`/token-sync`) |
...
Screens stocks with your own strategy code and places manual delivery orders. Every key is optional; the defaults are the ones shipped in `.env.example`.

| Key | Default | Meaning |
|---|---|---|
| `UI_SHOW_SWING_SCANNER` | `false` | Show the menu. Off by default — this page places **real** orders with no dry-run gate. |
| `SWING_SCANNER_MAX_ORDER_VALUE` | `1000000` | Fat-finger ceiling in ₹ on a **single** order's value. Over it, the order is refused before it reaches Zerodha and the refusal is logged. A typo guard, not a permission gate — raise it when you mean to trade bigger. |
| `SWING_SCANNER_SCALE_THRESHOLDS` | `true` | Re-express point-based strategy thresholds (`BB_RSI_MIN_BAND_WIDTH_PTS`, `BB_RSI_MAX_ENTRY_SL_PTS`, the `PA_*_PTS` family, `RSI_PIVOT_ST_PIVOT_BUFFER_PTS`) as the same **percentage of each stock's own price** that they represent on NIFTY. Without it a 50-point minimum band width is a third of a ₹150 share and nothing ever signals, while an 8-point stop on a ₹40,000 share is 0.02%. Both failures are silent. Turn it **off** to feed the raw point values through unchanged. |
| `SWING_SCANNER_NIFTY_REF` | `24000` | The NIFTY level your point settings were tuned at — the denominator in the rescale above. |
| `SWING_SCANNER_CONCURRENCY` | `4` | Parallel Fyers history calls during a scan. Higher is faster, but the broker rate-limits. |
| `SWING_SCANNER_CACHE_DAYS` | `7` | Retention for `~/trading-data/swing_scanner_cache/`. The cache is what makes a repeat scan near-instant. |
| `SWING_SCANNER_BROKERAGE_PCT` | `0` | Delivery brokerage % shown in the order popup — zero at Zerodha for CNC equity. |
| `SWING_SCANNER_STT_PCT` | `0.1` | STT % on a delivery buy. |
| `SWING_SCANNER_TXN_PCT` | `0.00297` | NSE transaction charge %. |
| `SWING_SCANNER_SEBI_PCT` | `0.0001` | SEBI turnover fee % (₹10/crore). |
| `SWING_SCANNER_STAMP_PCT` | `0.015` | Stamp duty % on a delivery buy. |

**Stock universe.** Three built-in presets — NIFTY 50, NIFTY 100, F&O (~228) — live in [src/utils/stockUniverse.js](src/utils/stockUniverse.js). Index constituents change and NSE revises the F&O list monthly, so any of them can be overridden, and extra watchlists added, via `~/trading-data/swing_scanner_universe.json`:

```json
{ "NIFTY50": ["RELIANCE", "TCS"], "MYLIST": ["IRCTC", "BEL"] }
```

Any key that is not one of the three built-ins becomes an extra entry in the dropdown. A key you leave out keeps its built-in list. A stale symbol is never silent — it appears in the scan's **Skipped symbols** panel with the reason.
...
| `TOKEN_SYNC_LIVE_URL` | — | **Laptop-only.** LIVE server the `/token-sync` **⇩ Pull from LIVE** button calls, e.g. `https://43.205.26.92:3000`. Blank = the button is disabled. Never set this on the LIVE server itself. |
| `TOKEN_SYNC_LIVE_LOGIN_SECRET` / `TOKEN_SYNC_LIVE_API_SECRET` | — / — | Credentials presented to that LIVE server. Blank = reuse this machine's `LOGIN_SECRET` / `API_SECRET` (correct when both boxes share one `.env`). |
| `TOKEN_SYNC_ALLOW_SELF_SIGNED` | `true` | Accept LIVE's self-signed TLS certificate on the pull. Turn off only if a real certificate is installed there. |
