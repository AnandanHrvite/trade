...
- **Debugging**: `ORB_DEBUG_TRACE=true` prints the **whole entry funnel** to the logs on every 5-min candle close — time window, OR, day sanity, breakout, body, VWAP, confirmation, retest — each PASS/FAIL/SKIP with its numbers and the final decision. The same trace always rides back on the signal as `sig.gates`, so the skip log records the full funnel rather than only the first blocking reason. Verbose; turn it off again after diagnosing.
- **Two stops fight each other — know which one binds.** `sig.slSpot` (the ATR stop) is 50–83 spot pts on typical volatility, but `ORB_MAX_TRADE_LOSS=1500` on a 65-lot ~0.6-delta option trips after only **~38 spot pts**. The rupee cap therefore ends essentially every losing trade and the spot SL never binds. Verified consequence: **every `ORB_SL_ATR_MULT` from 1.0 to 2.5 produces an identical result**, as does using the plain OR opposite edge. The multiplier is *inert*, not tuned — to change real risk, change `ORB_MAX_TRADE_LOSS`. Both routes log which stop binds at entry.
- **Expectation, stated honestly.** On the 39-session study, **with realistic option costs modelled** (delta 0.6, 1.5pt slippage per side, theta, charges) this design nets **~₹6,700 from 9 trades — of which one trade is ~81%.** Drop the top two and it is negative. Bootstrap 95% CI on mean-per-trade is **[−₹1,029, +₹2,998]**, i.e. roughly a **25% chance the true edge is zero or negative**. Establishing this edge at 95% confidence / 80% power needs **~147 trades ≈ 637 sessions ≈ 2.5 years**. The earlier "+346 spot points, no losers" figure was a spot-points artifact — in rupees it is 3 winners and 6 losers.
- **Nothing here is statistically proven.** Removals are justified structurally (a filter that deletes the entire right tail of a right-tail strategy is wrong regardless of p-value) and by the need to make the strategy measurable at all — with the prior-day filter on it took **1 trade per 39 sessions**, which can never be validated. The prior-day removal is Fisher-exact **p = 0.152**, i.e. not significant. Treat every constant as a prior to revise, not a fitted optimum, and collect clean paper sessions before enabling live.
