...
| `/all-backtest` | **Unified backtest dashboard** — runs the same date range across all enabled strategies and renders the per-strategy stats side by side. **🧹 Clear Cache** wipes the two historical-candle disk caches (`~/trading-data/backtest_cache` + `candle_cache`) so the next run re-downloads from Fyers — use it when a backtest looks like it ran on stale candles. Confirmed once, needs `API_SECRET`, and is refused while a backtest is running. Touches no trades and no settings. |
