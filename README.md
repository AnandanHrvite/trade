...
- **Exits**: EMA21 trail / EMA touch-back (or PSAR per `SWING_SL_MODE`), optional N-bar candle trail (`SWING_CANDLE_TRAIL_ENABLED`, tighter-of) · per-trade points stop (`SWING_STOP_LOSS_PTS`, off by default) · option-premium stop (`OPT_STOP_PCT`) · opposite signal · exit-before-close (`SWING_EOD_EXIT_TIME`) · EOD auto-stop (`TRADE_STOP_TIME`).
