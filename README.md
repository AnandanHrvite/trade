...
| **EarlyBird Paper** | The day's **first 15-min candle** on NIFTY sets the side; any F&O **stock** printing the same shape is bought/shorted on a breakout of its own candle — **CASH EQUITY, up to 5 positions** | 15-min (first bar) | Simulated | `/early-bird-paper` |
...
**This is the only strategy here that does not buy a NIFTY option.** It trades the **cash equity** of F&O stocks, INTRADAY product, a flat share count per name, and it holds **several positions at once**. NIFTY itself is never traded — the index only decides whether today is a buy day or a sell day. A PE-side signal is a real intraday **short sale**.
...
- **The 2% gap rule**: a stock opening more than `EARLYBIRD_MAX_GAP_PCT=2`% from its previous daily close is dropped. An **unknown** previous close is a refusal, not a pass — "we don't know the gap" is not "there is no gap".
...
- **Sizing**: `EARLYBIRD_QTY=100` **shares** (not lots) per name, at most `EARLYBIRD_MAX_CONCURRENT=5` at once. When more names confirm than the cap allows, the **tightest-stop** ones are taken. Note 100 shares of a ₹3,000 stock is a ₹300,000 position — size both keys deliberately.
- **Deliberately absent**: no VIX gate, no OI filter, no ADX/RSI/EMA/VWAP/SuperTrend, no volume filter, no confirmation candle, no trailing stop, no breakeven jump, no partial booking, no re-entry, and no `{MODE}_ITM_STEPS` — there is no option to pick a strike for.
