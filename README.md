...
- **Exit** (per-tick, **spot points**): **Profit lock** — once peak favourable spot move ≥ `SCALP_PROFIT_LOCK_TRIGGER_PTS(25)`, exit when it gives back below `SCALP_PROFIT_LOCK_PCT(50)`% of peak (ratchets up: peak 100pts → lock 50pts); the upside exit. → **Hard stop** — exit if the trade moves ≥ `SCALP_STOP_LOSS_PTS(30)` against entry; a **wide** catastrophic loss cap that only clips deep adverse excursions on failed fades (the shown PSAR SL is display/sizing only). Both points-based so they work even on spot-proxy sessions. → **Max-loss per trade** (per-tick, candle-gated): after `SCALP_MAX_LOSS_AFTER_CANDLES(3)` candles, exit if down ≥ `SCALP_MAX_LOSS_PTS(0=off)` — caps bleeders that slip the candle-close exits without touching the quick winners; no SL-pause. → **BB re-entry** (candle close): if price closes back inside the band (failed breakout) exit immediately (`SCALP_BB_REENTRY_EXIT`, default on) → **PSAR flip** on candle close handles trend runners → bid-ask spread guard → EOD. No break-even-to-entry snap, no PSAR/prev-candle SL trail, no % spot-trail, no time-stop.
...
| `SCALP_MAX_LOSS_PTS` | `0` | Per-trade loss cap that **only arms after `SCALP_MAX_LOSS_AFTER_CANDLES` candles** — caps genuine bleeders that slip past BB re-entry / trend-flip, without clipping the quick winners. Does **not** arm the SL pause (entry sequence unchanged). Points-based. `0` disables (default). |
| `SCALP_MAX_LOSS_AFTER_CANDLES` | `3` | The Max-Loss cap stays asleep this many candles after entry, then arms — so quick scalps that briefly dip against entry are never cut. |
