...
- **Days a strategy never ran on are replayable too.** In **Current-settings mode** the date-range card fills in every recorded day in the range that has no session marker for the chosen strategy (or, with *All strategies*, for each strategy independently) and replays the **whole recorded day** as a synthetic session — this is how a strategy added later is tested against days recorded before it existed. Those rows show `whole day (no session)`, have no live baseline to compare against, are never cached, and fetch their warm-up candles live (broker must be logged in). Snapshot mode skips them: a marker-less day has no recorded settings to replay with.
