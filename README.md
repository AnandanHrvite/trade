...
> **Phases A + B ship now.** Paper (`/trend-pb-paper`) and Backtest (`/trend-pb-backtest`) are live; live trading (`/trend-pb-live`, dry-run gated) is Phase C, after paper is validated. Design doc: reviewed & approved before implementation, then adversarially code-reviewed (institutional-grade single-strategy build — capital preservation over trade frequency, ≤ ~7 real signal knobs).
