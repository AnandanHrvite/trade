...
- **Top-bar PAPER/LIVE toggle + Range selector** — one pair of controls drives every chart on the page. Range offers *This month*, *Last month*, *Current week expiry*, *All* (default) and *Custom* (From/To) — the same five as Edge Analytics, from one shared definition in [sharedNav.js](src/utils/sharedNav.js). It filters the cumulative chart and all per-module cards (counts, W/L and net included) client-side — no refetch. *Current week expiry* runs from the day after the previous NIFTY weekly expiry through the current one, reading `/api/expiry-dates` so a holiday-preponed expiry is honoured.
...
| `EMA_RSI_ST_OPTION_EXPIRY_OVERRIDE` | (blank) | EMA_RSI_ST-only expiry override — keep EMA_RSI_ST on next-week expiry while bb_rsi/PA trade current. Blank inherits the common expiry. Saving this field alone is independent; saving the **common** expiry overwrites it (see the expiry fan-out note below). |
