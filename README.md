...
- **2026-08-11 — the wide sample says the same thing, and says ORB is not profitable.** Two backtest runs (2025, n=122; 2026 Jan–Jul, n=59) came in at **−₹51,420 (23% win, PF 0.52)** and **−₹32,268 (20% win, PF 0.38)**. Splitting them by opening-range width reproduces the hypothesis above on 181 trades instead of 9: **OR ≤ 70pt** was +₹10.6k (2025) / −₹3.4k (2026), **OR > 70pt** was −₹62k / −₹28.9k, ~14% win in both. Nothing else (side, entry time, plain vs retest vs resume) held across both years — `OR ≤ 60` and "drop plain entries" both looked excellent on 2025 and collapsed on 2026, which is what curve-fitting looks like. So the ceiling found so far is **break-even, not profit**; `ORB_OR_MAX_PTS` exists for it and ships **off**. Sweep configs with `node scripts/orbSweep.js --from … --to …`, which forces a train/test split for exactly this reason.
...
| `ORB_OR_MAX_PTS` | `0` (off) | Absolute cap on opening-range width in points. OR width was the only cut that separated winners from losers in **both** the 2025 (n=122) and 2026 (n=59) samples — ≤70pt was roughly break-even, >70pt lost heavily in both. Ships **off**: it turns a big loser into a scratch, which is not an edge. Measure with `scripts/orbSweep.js` first |
