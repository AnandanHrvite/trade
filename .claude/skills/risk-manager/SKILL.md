...
- Live order placement is TRIPLE-gated (`src/utils/liveDryRun.js`): global `LIVE_HARNESS_DRY_RUN` (default true), per-strategy `{KEY}_LIVE_ENABLED` (default false), per-strategy `{KEY}_LIVE_DRY_RUN` hold-back override. Keys are the mode names uppercased, with four short forms — `TDS`, `GAP3M`, `OIWF`, `EMA9VWAP` — so a guessed key silently reads an unset variable and reports the SAFE default. Every layer can only add safety. Treat `LIVE_HARNESS_DRY_RUN=true` as the primary kill switch for live risk.
...
- The portfolio breaker exists but ships OFF: `PORTFOLIO_MAX_DAILY_LOSS` (default 0 = disabled) sums today's realized paper P&L across all thirteen modes and blocks new entries only, fail-open. It caps REALIZED loss; **nothing caps simultaneous OPEN risk**, so thirteen engines taking the same-direction option on one shared feed remains the largest uncovered correlation risk in the system — and it grows with every strategy added. Name it in any risk review.
- `capitalPool.js` is ADVISORY ONLY — it never stops a trade and the pool is allowed to go negative. It also maps only ELEVEN of the thirteen strategies: `trend_day_scalp` and `gap_fix_3m` have no row, and the paper route calls check/block/release with the mode key regardless, so for those two all three are silent no-ops and their capital is never reserved. Re-read the `STRATEGIES` table before citing the pool as cover for any strategy.
