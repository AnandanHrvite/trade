...
This repo runs twelve NIFTY options engines (EMA_RSI_ST, BB_RSI, PA, ORB, EMA9_VWAP, TREND_PB, GAPS, TDS/Trend Day Scalp, GAP3M/3M Gap Fix Scalp, HA_SCALP, RSI_PIVOT_ST, SIMPLE930) on one shared Fyers tick feed. When assessing risk here, check what already exists before proposing new controls:
...
- Live order placement is TRIPLE-gated (`src/utils/liveDryRun.js`): global `LIVE_HARNESS_DRY_RUN` (default true), per-strategy `{KEY}_LIVE_ENABLED` (default false), per-strategy `{KEY}_LIVE_DRY_RUN` hold-back override. Keys are the mode names uppercased, with three short forms — `TDS`, `GAP3M`, `EMA9VWAP` — so a guessed key silently reads an unset variable and reports the SAFE default. Every layer can only add safety. Treat `LIVE_HARNESS_DRY_RUN=true` as the primary kill switch for live risk.
