...
On PE the line is **mirrored**, not reused: it must sit ABOVE price (bearish) to be a valid stop, it ratchets DOWN instead of up, and a flip to *bullish* — not bearish — is the exit. Entry, the candle-close trail, the per-tick test and the backtest all follow the same mirror, and the backtest now tests the SuperTrend against the bar's adverse extreme per side (the high for a PE, not the low) with the same open-through-the-level fill rule the premium floor already used.

**Enabling it on PE makes that side trade less, not more.** A side carrying the SuperTrend also inherits the refusal rule: a setup whose line is on the wrong side of price, or already at/through the entry close, is rejected rather than entered without a stop.
...
**Read this before switching it off for PE.** At the defaults a PE carries *no other stop*, because the SuperTrend defaults to CE only. Excluding PE therefore leaves every PE trade completely unstopped, with the `RSI_PIVOT_ST_EXIT_TIME` square-off as its only exit and the full premium at risk. The same applies to any side that has neither stop — see `RSI_PIVOT_ST_ST_SIDES` above. The engine still takes those trades — the toggle does what it says — but it refuses to be quiet about it: the paper route warns at start-up *and* on every such entry, the live-harness page prints a red banner, the backtest notes name the affected side, and the crash-recovery alert flags a recovered position that has no stop at all.
...
An entry needs **both halves on the same closed 5-minute candle**. A CE requires `RSI(14) > 70` **and** a candle that *crosses and closes* above R1 — the previous close at or below the level, this one above it. A candle already sitting above R1 is not a cross, and entering there is chasing a move that already happened, so it is refused. A PE is the mirror: `RSI(14) < 40` and a candle crossing and closing below S1. Reading the RSI on the very bar that does the crossing is deliberate — it removes any lag between the two tests. The strike is `RSI_PIVOT_ST_STRIKE_PCT=1`% of spot in the direction `RSI_PIVOT_ST_STRIKE_MODE=OTM` picks, rounded to the nearest 50-point strike, with a fallback to ATM when 1% rounds to zero steps rather than inventing a strike.

**The two sides are stopped differently, on purpose.** A CE carries **two** stops, both live, whichever triggers first: a `SuperTrend(10, 2)` line on the 5-minute spot chart that serves as both the initial SL and the trail — it only ever ratchets in the trade's favour, and a flip to bearish is itself an exit — plus a premium floor at `RSI_PIVOT_ST_PREMIUM_SL_PCT=25`% below the trade's **high-water** option price. A PE carries **only** the premium floor; there is no SuperTrend on the PE side. This asymmetry is the stated rule rather than an oversight, and it is written into the engine header, the paper route header and the strategy guide so a later reader does not "balance" it. A CE setup whose SuperTrend is not bullish, or whose SuperTrend already sits at or above the entry close, is refused rather than entered with no stop.

There is **no profit target at all** on either side. The trade runs until a stop trails into it or `RSI_PIVOT_ST_EXIT_TIME=15:15` squares it off — no breakeven jump, no partial booking, no time stop, no re-entry after a stop. Equally deliberate is what is absent: no VIX gate, no OI filter, no ADX, no volume test, no EMA, no VWAP, no ATR sizing, no multi-timeframe bias, no extra confirmation candle, no expiry-day rule.

Everything the *decision* reads is a closed 5-minute bar or the previous day's completed daily bar — never a live tick. The premium stop is the necessary exception, because it reads the option LTP, but it reads it only for the exit and those ticks are recorded so Replay reproduces them. Data comes from Fyers; the **orders go to Zerodha**, like EMA_RSI_ST, which is why `/rsi-pivot-st-live/start` checks both brokers. Live wraps Paper through the shared harness (Live = Paper by construction) and is triple-gated to dry-run — `RSI_PIVOT_ST_LIVE_ENABLED=true` AND `LIVE_HARNESS_DRY_RUN=false` AND `RSI_PIVOT_ST_LIVE_DRY_RUN` not-true. An open position is crash-recovered via `positionPersist` (`.active_rsi_pivot_st_position.json`).

The backtest drives the same `getSignal` and re-implements only paper's exits, with the conservative intra-bar ordering the other engines use: the adverse stop is tested on the bar's high/low before anything favourable, a bar that opened beyond a level fills at the open, and the SuperTrend trail advances only on a bar close. It fetches two series — the intraday bars for decisions and a **daily** series padded a week before the range, so the first session still has a yesterday to compute R1/S1 from. **Its rupee figures are the least trustworthy part of it**: there is no historical option chain, so the premium is δ+θ simulated seeded at `RSI_PIVOT_ST_BT_SEED_PREMIUM=180`, and the 25% floor is applied to that simulation rather than to a real quote.

**It has never traded — not one paper session, not one live order — and no backtest has been run against it.** Every threshold is the stated rule or a repo convention, not a fitted value. Settings ships the full schema under *RSI_PIVOT_ST Strategy*, the sidebar group and per-page toggles are gated by `RSI_PIVOT_ST_MODE_ENABLED`, and [documents/RSI_PIVOT_ST_Strategy_Guide.html](documents/RSI_PIVOT_ST_Strategy_Guide.html) is the plain-English walkthrough — its charts are drawn from output of the real engine, not hand-invented.
...
Kite Connect has no historical order/trade endpoint — `/orders` and `/trades` are transient, today-only ("the order history ... only lives for a day in the system," confirmed against the official docs; Kite MCP has the identical limit) — so past manual trades are seeded via a one-time **Kite Console → Reports → Tradebook CSV import** button, which now accepts **multiple files at once** (imported sequentially so concurrent writes to the same store file can't race) and lets a re-imported file **replace** its previously-stored rows in place instead of silently skipping them, so a corrected re-export always wins. Going forward, today's fills **auto-sync daily at 15:35 IST** (`MANUAL_TRADES_AUTO_SYNC_ENABLED`, default on, same self-rescheduling-timer pattern as the consolidated EOD reporter) or on-demand via **Sync Now**. Fills are FIFO-matched into round-trips per symbol in `utils/manualTrades.js`; segment classification trusts the CSV's `segment`/`exchange` column first — symbol-suffix matching alone is unsafe (equity names like RELIANCE end in the letters "CE" and would misclassify as a Call Option). Data lives at `~/trading-data/manual_trades.json`, independent of the bot's own strategy trade logs. Added `zerodhaBroker.getTrades()` to fetch today's fills. Raised the global JSON body limit from Express's 100kb default to 25mb — a full year's F&O tradebook CSV is well over 100kb and was failing the import with "request entity too large".
...
Next to it, **🧹 Clear tokens here** (`POST /token-sync/reset`) is the undo: it deletes this machine's `.fyers_token` / `.zerodha_token` and drops them from `process.env`, so a bad or stale credential is gone rather than sitting there to be half-used by the next backtest. It touches only the machine it runs on, and it is refused while any engine holds the socket — pulling a running feed's token out from under it is not an accident worth allowing.
...
A **♻ Restart app** button covers the case where `.env` also changed. It is refused while any engine holds the shared socket — tested against `isAnyActive()` rather than `getMode()`, which knows only about EMA_RSI_ST and would have let a restart orphan a running BB_RSI, PA or ORB session — and the page says plainly that under a plain `npm start` the process will stop rather than come back (PM2 and nodemon restart it).

The page places no orders and touches no trade state; it sits behind the same `LOGIN_SECRET` gate as every other page, and the raw token is only rendered on an explicit click. Only the page shell is an open path, because a browser navigation cannot carry the `x-api-secret` header; the shell renders no token. Everything that touches a credential — the token poll as much as **apply** and **restart** — requires `API_SECRET` and goes through `secretFetch`. Gating a GET that way is deliberate: the poll returns a live broker token, so the login cookie alone should not be enough to read it off a screen. A token read from `process.env` with no file behind it is badged *age unknown* rather than *valid today*, since nothing on disk says when it was issued. Gated by `UI_SHOW_TOKEN_SYNC` (default on) in Settings → Menu Visibility.
...
**Settings → BB_RSI → `BB_RSI_RESOLUTION`** — `global` (default) / `3` / `5`. `global` follows `TRADE_RESOLUTION`, so an untouched `.env` behaves exactly as before; nothing changes unless you pick a value. Applies to Live, Paper and Backtest together — the value is resolved once in `src/strategies/bb_rsi.js`, so the three modes cannot drift onto different candle sizes the way the old per-strategy keys allowed. Paper picks it up on the next session start and Backtest on the next run; BB_RSI **Live** caches it at module load, so a change there needs a full server restart, exactly as `TRADE_RESOLUTION` always has.

The backtest's own candle-counted rules — the SL pause window and the confirmation candle's "is this the immediately-next bar" test — now measure the **actual** bar spacing of the run instead of reading the config. Those two can legitimately disagree: an explicit `?resolution=` beats the env, and `/all-backtest` sends a fixed 5 to every panel. `confirmCandle.isNextBar` compares bar gaps for exact equality, so a configured `3` against 5-min bars matches nothing and the confirmation candle (on by default) would have blocked **every** entry — a silent 0-trade run with no error. The theta calculation already derived its own spacing for this reason; the rest of the run now shares it.

That measurement is also taken more carefully than theta's was. It reads the **smallest** gap in the opening stretch rather than just the first one: the first pair straddles an overnight break whenever the range's first day carries a single candle, which used to cost only a skewed theta divisor but would now read ~1080 min and take the whole run to zero trades. Intraday bars are uniformly spaced, so every larger gap is a session boundary and the minimum is the honest reading.
...
Every numeric setting the engine reads is now parsed **strictly** and falls back to its documented default on a malformed value. This was a real hole in the new guards: a `NaN` threshold makes every comparison against it false, so a typo in `BB_RSI_MIN_BAND_WIDTH_PTS` switched the chop guard **off** rather than on, and `parseFloat("5o")` quietly returned `5` for an intended `50`. The entry window had the same shape — a malformed `HH:MM` produced `NaN` bounds, both window comparisons went false, and the engine traded the entire session. Explicit `0` still parses cleanly, so every "`0` = off" opt-out is unchanged, while the structural inputs (BB/RSI periods, divergence lookback and pivot bars, and the two candle counts) carry a floor — `0` means nothing there, and unfloored it either throws out of the Bollinger calculation or makes an opposite-candle stop fire on the first close of every trade. Same ruling as `boundedExit`'s `LIVE_EXIT_WAIT_MS` and EMA9+VWAP's window.
...
- [utils/expiryHealth.js](src/utils/expiryHealth.js) now runs a **post-close ladder**: 15:40 first, then 16:15 / 16:30 / 16:45 while the expiry is still stale. Fixed minutes, not interval ticks, because a 30-minute tick anchored to boot could fall at 15:35 and then at 16:05. The scheduler is now a one-minute heartbeat; the in-window checks still honour `EXPIRY_HEALTHCHECK_MINS`. On a normal day the ladder costs nothing — with a live expiry at 15:40 it stands down without a broker call.
- The **16:00 EOD token clear waits** while that ladder is running ([app.js](src/app.js)) — every retry needs the Fyers token the clear destroys — re-asking each minute up to a hard 16:55 deadline, so a wedged check can never leave a token alive overnight.
- The Dashboard says which of the two it is: an amber *"expired — updating it automatically"* strip while the roll is retrying, and the red *"entries are blocked"* alert only after all four attempts have failed. One Telegram goes out at that point, not before.
...
A walk over URLs only sees the screens a URL can reach, so the states behind them were covered separately: the auth success/error/token cards and the four backtest error pages were rendered directly in both themes, which turned up a message strip that stayed black in the light skin, two action buttons whose fill was set by a variant rule that inherited a white label, and card paragraphs with no light rule at all. For the states that need live data — the trade-detail modal, an open position, an empty table — rendering is not possible at all, so those were closed by a surface-independent rule instead: every dark surface in the app has relative luminance ≤ 0.03, so any text below L=0.175 fails 4.5:1 against all of them by construction. That found 66 more declarations, including the modal's "🟢 Entry" / "🔴 Exit" headings at roughly 1.6:1 across twelve files.

One class of button was still left, because a rendered pass never hovers: a `:hover` fill is only painted while the pointer is on the control, so every hover colour the rewriter has no map entry for went unmeasured and stayed on its dark-theme value. Hovering the Consolidation Report button, the Settings Advisor's re-analyse button or the per-pattern Copy button turned each one into a dark block under a label the light skin had already darkened — the worst landed at 1.26:1. One resting state was wrong for the mirror-image reason: the broker strip's two login buttons take their fill from a class rule and their label from `themeJS`, and the rewriter has an entry for the Fyers green but none for the Zerodha navy, so that button rendered #1d4ed8 text on a #1a4a8a fill at 1.25:1. Both states of all of them are now written as explicit `:root[data-theme="light"]` rules — the same treatment `allBacktest`'s Run button already had, now applied to the four backtest pages that were missing it — and measured in headless Chrome at rest and with `:hover` forced on: every one clears 4.8:1 in both states.
...
Also fixed by the same helper: `getLastTuesdayOfMonth()` returned a weekly-format code for what is by definition the monthly contract (so the monthly fallback step could never validate); the monthly step now rolls to next month once this month's expiry has passed (measured against the nearest still-tradeable weekly date, so it rolls at 15:30 on expiry day, not at midnight); the Market Context Snapshot that Replay and the option-chain recorder pin their expiry from was inheriting all of the above.
...
Each contract block is now fetched **independently**. What Fyers still serves is used; what it refuses is recorded with a translated reason (`contract expired — Fyers no longer lists it`, not a raw API dump) and reported on the results page as **partial coverage**, naming the missing symbols and their date ranges. That distinction matters: a silently absent month reads as "nothing traded then" when it actually means "never looked".

REFUSED and EMPTY are deliberately kept apart, because merging them recreates a misdiagnosis this repo has already paid for once. A *refused* block means Fyers rejected the symbol and those sessions can never be fetched. An *empty* block means the call succeeded and returned nothing — normally a window with no trading days in it, but **also what an expired token looks like**, since Fyers answers `no_data` rather than an auth error. So an all-empty run is never blamed on delisting: it points at the token. An empty block never raises the amber partial-coverage warning either, though it is still disclosed. Only when every contract was genuinely *refused* does the failure lead with the delisting explanation, offer the current contract's window as a working range, and keep the token hint as the secondary cause.
...
Unsubscribing is instant locally but not on the wire, so a dropped contract keeps delivering whatever was already queued. Those stragglers match neither the subscribed set nor the spot symbol, and would land in the spot fan-out — on essentially every trade exit. Dropped symbols are therefore tombstoned as known-not-spot for a minute (cleared if the same strike is re-entered), tombstones outlive a session teardown so a straggler cannot be learned as the *next* session's spot symbol, and delivery to strategies is a positive match against the learned spot symbol rather than "anything that wasn't an option". That last check stays dormant until an option actually shares the socket, and again after a bail-out, so a session that never streams an option is byte-for-byte unchanged.

Dropping a tick is the right answer to one odd tick, but a sustained run of them would mean the spot feed itself had stopped reaching the strategies — a silent trading halt the existing watchdog cannot see, because ticks *are* still arriving. So 50 consecutive unattributable ticks with no spot delivery in between abandons option sharing for the session outright: the contracts are unsubscribed, the probe resets, and the connection goes back to carrying the index alone while engines revert to REST.

The watchdog needed the same correction from the other direction. It reconnects on tick silence, and that was a sound proxy for "the spot feed is alive" only while every tick *was* a spot tick. Option traffic would have kept its clock fresh through a dead spot subscription, so it now measures silence on ticks actually delivered to the strategies. `/auth/socket-health` reports both clocks.
...
New keys `OPTION_SOCKET_FEED_ENABLED` (default on, instant kill-switch), `OPTION_SOCKET_FRESH_MS`, `OPTION_SOCKET_LEASE_MS` and `OPTION_SOCKET_RECORD_MS`, all in Settings under "Option Price Feed". Covered by 30 assertions in `scripts/optionFeedTest.js`, most of them aimed at the failures that would be unrecoverable rather than merely disappointing.
...
**The chart it reads was decided by measurement, not preference.** The natural instinct is to run this on the NIFTY 50 index like every other engine here. Measured first, on this repo's own cached index candles across 39 sessions: **12 intraday gaps in total, the largest 2.1 points, median 0.45**. The reason is structural rather than incidental — an index is a continuously recomputed average of 50 stocks, it has no order book, and an average never leaves a hole. NIFTY **futures** is one traded contract with a real book, and that book is what gaps when it thins out. So this is the first engine in the repo whose decisions read futures candles. The index is still used for exactly two things: choosing the option strike (strikes are struck on the index, not the future) and the second chart on the Paper page. No rule reads it.

**The trade.** A gap of ≥20 points is remembered exactly where it is. The next candle then decides: if it breaks the day's high (gap up) or low (gap down) on ≥1.5× the recent average futures volume, the breakout is real and **nothing is traded**. If it turns back instead, the gap is faded — PE on a gap up, CE on a gap down — with the target set at the gap's fill level and the stop at the day's extreme, frozen as of the gap bar. Neither level ever moves: no trail, no breakeven jump, no partials, no time stop, no premium stop. Two levels and an EOD square-off.

**Where the volume rule fails safe rather than open.** If the futures candles come back without volume — an expired token, a data hiccup — the engine cannot demonstrate that a break of the day's extreme was weak. It then treats every such break as real and stands aside. Fading a genuine breakout is the expensive mistake here, so unknown is read as "assume real", never as "assume weak".

**Why the minimum gap is 20 points.** The gap size *is* the target — you cannot make more than the gap. On one lot, this repo's own `charges.js` takes ~₹90 round-trip plus ~1.5 premium points of slippage each way, which at delta 0.6 is **~7.3 index points gone** before the trade has made anything, and a target has to clear that with room to spare. Below roughly 17 points the strategy is negative-expectancy by arithmetic whatever its win rate turns out to be. Twenty is the first round number safely clear; it is a floor derived from cost, not a value fitted to anything.

**The geometry finding, which matters more than any threshold.** Target and stop are not independent: the target is the gap's far edge and the stop is the day's extreme, so **target + stop is a fixed span** and the confirm candle's close decides how it is split. Run on one 22-point gap with only that close varied, the engine returns reward-to-risk anywhere from **3.75** (a shallow return, stop close by) down to **0.27** (a deep return back inside the void, stop far away). The convincing-looking trade is the worse one. That is why `GAP3M_RETURN_MODE` ships as `reverse_close` rather than `into_gap`, and it is documented in the guide with the engine's own numbers rather than left to be discovered live.

**Three guards ship switched OFF on purpose.** `GAP3M_MIN_RR`, `GAP3M_MAX_SL_PTS` and `GAP3M_MAX_EXTREME_DIST_PTS` all default to 0, so out of the box the engine does exactly and only what the rules say — including taking a trade whose stop is a long way off because the gap formed far from the day's extreme. They exist as Settings levers for once there is real data to justify a number, rather than as a guess baked into a default.

**Data plumbing worth knowing before reading a fill price.** Closed 3-minute futures bars come from the Fyers history endpoint, refreshed once per bar a few seconds after it closes — the same endpoint the backtest and replay read, which is what makes the four modes agree on a session. The live futures price the two exit levels are checked against comes from a quote poll every 2 seconds, because the shared tick socket carries the index rather than the future. **Exits therefore resolve at ~2-second granularity, not tick by tick.** The backtest additionally splits any date range into front-month contract blocks using the same roll rule as the live path, so bars from two different contracts are never adjacent inside a session and a contract roll can never be mistaken for a gap.

Ships with Paper (canonical), Backtest and a Live harness triple-gated to dry-run, wired into Settings, the sidebar, Real-Time, both consolidations, Edge Analytics, Logs, Cache Files, Replay and the unified backtest page, plus a strategy guide whose three charts and every quoted number are produced by running the real engine. Verified with 146 offline assertions, a full app boot with every route curled, a hide-when-disabled check across all seven shared screens, a live-settings check confirming both the strategy config and the chart feed move, and a three-way check that all 37 env keys agree across code, Settings and README with no dead keys and no rule logic duplicated outside the engine.

**Not validated.** Zero paper sessions, zero live orders — and unlike the other eight strategies, even the trade *frequency* is unknown, because the futures gap distribution it depends on could not be sampled at build time (the Fyers token was expired). Collect clean paper days and diff them against `/replay` before touching any live gate.

### Added — TREND DAY SCALP, a strategy built to be boring on purpose

Every other engine in this repo is the same trade wearing different clothes: buy a breakout on a 5-minute close, let the winner run. That shape is right-tail dependent, and the numbers say so. ORB's own header records 9 trades over 39 sessions where the single best is 211% of the net — delete that one trade and the strategy is −₹3,786. EMA9+VWAP's +₹16k was one trade; without it the sample is −₹1,481. Six strategies, one opinion, and the result hangs on catching a monster.

**TREND_DAY_SCALP inverts that deliberately.** It gives up the huge winner in exchange for a result that does not need one: a fixed target, a stop that is always the same size, and a day filter whose main job is to produce a **zero** rather than a loss.

The rule that shaped everything else is arithmetic, not taste. Measured with this repo's own `charges.js` on one lot: ~₹90 of statutory charges plus ~1.5 premium points of slippage per side (~₹225) is **~₹315 gone before the trade is right about anything** — about 17 index points of edge at delta 0.6 just to break even. That is why the target floor is 2.5R rather than a scalper's ten points; an average win of twelve points cannot survive its own costs however often it lands.

**How a day goes.** At 10:15 the engine judges the first hour once and freezes the verdict: the range must be ≥0.5% of spot, the last six closes must all sit on one side of VWAP, and spot must be extended ≥0.35× the range from VWAP. Fail any one and the day is over — no trade, no loss. Pass all three and the direction is **locked** (above VWAP → CE only) for the rest of the session. It then waits for a pullback into the nearer of VWAP / EMA20 that gets reclaimed on the close with a body ≥0.4×ATR. The stop is the pullback extreme, floored at 12 points and **skipped entirely above 18** — never widened, never tightened into the structure, so every loss is the same size. Target is a fixed 2.5R. The stop moves exactly **once**, to breakeven at +1R, and then never again.

That last point is the deliberate break from the rest of the repo: **there is no rolling trail.** A rolling trail is precisely what turns the other engines' winners into scratches, tapping out on a normal pullback just before the move continues. Here the fixed target does that job.

**One threshold changed during the build, and the reason is worth recording.** The extension gate was specified at 0.6× the range. The offline harness rejected every clean trend day, and the arithmetic explains why: on a perfectly linear ramp the time-weighted VWAP lands at (L+H)/2, so spot−VWAP is *exactly* 0.5× the range. A 0.6 threshold therefore admits only days that accelerate into 10:15 — a handful per year. It ships at **0.35**, which still separates a committed day (~0.5) from a chopping one (~0.1).

Also new: day-level breakers sized for a steady curve rather than a big one — max 2 trades, day ends on 2 **real** stop-outs (a breakeven or time-stop exit is a scratch and does not count), ₹1,500 loss cap, and a ₹1,500 **profit lock**, because handing profit back is what wrecks a low-but-steady result.

Ships with Paper (canonical), Backtest and a Live harness triple-gated to dry-run, wired into Settings, the sidebar, Real-Time, both consolidations, Edge Analytics, Logs, Replay and the unified backtest page, plus a strategy guide whose charts are generated by running the real engine. Verified with 226 offline assertions, a full app boot with every route curled, a live-settings check confirming both the strategy config and the chart feed move, and a three-way check that all 32 env keys agree across code, Settings and README.
...
The page is built for a phone as much as a desktop. On an iPhone 17 Pro Max (440 px) the stat cards drop to two columns, the filters and the Re-analyse button become 44 px targets with 16 px selects so iOS stops zooming the page in on focus, content clears the fixed hamburger, and the horizontal padding respects the safe-area insets. The By-Strategy table is wider than a phone, so it gets its own sideways scroller with a swipe hint that only appears when it is actually cut off — measured, not assumed: with `width:100%` alone the table box stayed at the wrapper's 390 px while a 573 px cell spilled past it and was silently clipped, which `min-width:max-content` fixes. Touch sizing keys off `pointer: coarse` rather than width alone, because the same phone in landscape is 956 px wide and a width-only breakpoint left it with 31 px targets.
...
Two backtest pages were hardcoded to re-run at 5-min regardless of the setting: the **Run Again** bar on the PA and BB_RSI backtests carried a hidden `resolution=5`, so a 3-min (and now 15-min) run silently became a 5-min one on the second click. It now carries the run's own resolution, and the per-pattern PA backtest's *PA Candle* dropdown gained the 15-min option so it can show the global value.
