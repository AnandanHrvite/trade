...
New keys `OPTION_SOCKET_FEED_ENABLED` (default on, instant kill-switch), `OPTION_SOCKET_FRESH_MS`, `OPTION_SOCKET_LEASE_MS` and `OPTION_SOCKET_RECORD_MS`, all in Settings under "Option Price Feed". Covered by 29 assertions in `scripts/optionFeedTest.js`, most of them aimed at the failures that would be unrecoverable rather than merely disappointing.
