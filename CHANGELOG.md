...
- **Scalp & Swing history routed through the shared builder** — both now call `renderHistoryPage()` and their ~2,100 lines of duplicated inline page HTML/JS were deleted (single source of truth). **PA kept its own page** because it carries a pattern-attribution filter (filter session cards + recompute stat cards/Day View/Analytics by detected pattern) that the generic builder does not implement; it still uses the shared theme + server-side pagination.
