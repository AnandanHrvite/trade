...
async function simulateBuy(side, sig) {
  ...
  const _cap = capitalPool.gate(MODE_KEY, instrumentMode.capitalRequired(qty, optionEntryLtp, UNDERLYING), { side, symbol: optInfo.symbol, qty });
  if (!_cap.ok) {
    if (!_cap.muted) {
      log(`❌ [BN-PIVOT-RSI-ST-PAPER] Entry REFUSED — ${_cap.reason}`);
