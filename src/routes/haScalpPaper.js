...
async function simulateBuy(side, sig) {
  ...
  if (!_cap.ok) {
    if (!_cap.muted) {
      log(`❌ [HA-SCALP-PAPER] Entry REFUSED — ${_cap.reason}`);
      skipLogger.appendSkipLog(MODE_KEY, { gate: "capital", reason: _cap.reason, spot: spotPrice, side, symbol: optInfo.symbol, qty, cost: _cap.cost, available: _cap.available });
    }
    return;
  }
...
router.get("/status/data", (req, res) => {
  ...
  if (pos && state.optionLtp != null) {
    livePnl = instrumentMode.unrealisedPnl({
      ...
      entryPremium: pos.optionEntryLtp, currentPremium: state.optionLtp, qty: pos.qty || haLotQty(),
    });
  }
