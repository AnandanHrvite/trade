/**
 * BN_EMA_RSI_ST_V2 BACKTEST — /bn_ema_rsi_st_v2-backtest
 * ─────────────────────────────────────────────────────────────────────────────
 * Date-range backtest on 5-minute NIFTY BANK INDEX candles for BN_EMA_RSI_ST_V2.
 *
 * The NIFTY BANK twin of /ema_rsi_st_v2-backtest. Same rules from the SAME shared
 * engine — every call below passes prefix "BN_EMA_RSI_ST_V2", so this page reads
 * the BANKNIFTY tuning and never the NIFTY one. The index, its 100-pt strike grid
 * and its 30-lot size come from instrumentConfig.underlyingOf("BANKNIFTY").
 *
 * BANKNIFTY OPTIONS ARE MONTHLY-ONLY (NSE withdrew the weeklies in Nov-2024), and
 * that changes what these rupees mean. A monthly contract decays FAR more slowly
 * than a weekly, so the θ this sim charges (BACKTEST_THETA_DAY, a NIFTY-weekly
 * figure) OVERSTATES the real decay here, while BANKNIFTY's roughly 2× larger
 * point moves mean the same SuperTrend distance is a bigger rupee risk. Nothing
 * in this file compensates for either: the guard defaults below are the NIFTY
 * version's numbers verbatim. Treat the ₹ column as directional only.
 *
 ...
 *   3. Hard square-off at BN_EMA_RSI_ST_V2_EOD_EXIT_TIME (default 14:00 IST).
 ...
 *   • a spread/slippage haircut of BN_EMA_RSI_ST_V2_BT_SLIPPAGE_PTS is applied EACH
 *     way — without it a backtest of option BUYING always flatters.
 *
 * CONFIRMATION CANDLE (BN_EMA_RSI_ST_V2_CONFIRM_CANDLE_ENABLED, default ON): the
 ...
 * day or two. A runway of BN_EMA_RSI_ST_V2_WARMUP_DAYS (default 5) calendar days is
...
const ACCENT      = "#2563eb";
const ENDPOINT    = "/bn_ema_rsi_st_v2-backtest";
const MODE_KEY    = "bn_ema_rsi_st_v2";
// The engine is SHARED with the NIFTY variant and is prefix-parameterised, so
// EVERY engine call must carry this. Omitting it silently reads the NIFTY
// EMA_RSI_ST_V2_* settings and the BANKNIFTY strategy would trade NIFTY's tuning.
const PREFIX      = "BN_EMA_RSI_ST_V2";
const LABEL       = "BN_EMA_RSI_ST_V2";
const RESULT_KEY  = "BN_EMA_RSI_ST_V2_BACKTEST";
// The index this backtest fetches. Read from underlyingOf("BANKNIFTY") rather
// than typed out, so the strike grid / lot size / monthly-expiry facts and the
// spot symbol can never disagree.
const UNDERLYING  = "BANKNIFTY";
function _u() { return instrumentConfig.underlyingOf(UNDERLYING); }
const SPOT_SYMBOL = _u().spot;   // NSE:NIFTYBANK-INDEX
const PAGE_TITLE  = "BN_EMA_RSI_ST_V2 Backtest";
...
function getGuardConfig() {
  return {
    entryStartMin:  _parseMins("BN_EMA_RSI_ST_V2_ENTRY_START", "10:30"),
    entryEndMin:    _parseMins("BN_EMA_RSI_ST_V2_ENTRY_END",   "13:00"),
    eodExitMin:     _parseMins("BN_EMA_RSI_ST_V2_EOD_EXIT_TIME", "14:00"),
    maxDailyTrades: _int("BN_EMA_RSI_ST_V2_MAX_DAILY_TRADES", "2"),
    maxDailyLoss:   _num("BN_EMA_RSI_ST_V2_MAX_DAILY_LOSS",   "1000"),
    maxConsecLosses: _int("BN_EMA_RSI_ST_V2_MAX_CONSEC_LOSSES", "2"),
    slPauseCandles: _int("BN_EMA_RSI_ST_V2_SL_PAUSE_CANDLES", "2"),
    oppCooldownEnabled: _bool("BN_EMA_RSI_ST_V2_OPPOSITE_SIDE_COOLDOWN_ENABLED", "true"),
    oppCooldownCandles: _int("BN_EMA_RSI_ST_V2_OPPOSITE_SIDE_COOLDOWN_CANDLES", "2"),
    warmupDays:     _int("BN_EMA_RSI_ST_V2_WARMUP_DAYS", "5"),
    slippagePts:    _num("BN_EMA_RSI_ST_V2_BT_SLIPPAGE_PTS", "2"),
    seedPremium:    _num("BN_EMA_RSI_ST_V2_BT_SEED_PREMIUM", "180"),
    resolutionMins: _int("TRADE_RESOLUTION", "5"),
    confirmEnabled: confirmCandle.enabled("BN_EMA_RSI_ST_V2"),
  };
}

// ── The engine loop ─────────────────────────────────────────────────────────
/**
 * runBnEmaRsiStV2Backtest(candles, activeFromTs)
 *
 ...
 * swallowed.
 */
function runBnEmaRsiStV2Backtest(candles, activeFromTs) {
  const empty = {
  ...
  const stratCfg = engine.getConfig(PREFIX);
  ...
  const LOT_SIZE  = instrumentConfig.getLotQty(UNDERLYING);   // 30 on BANKNIFTY, never NIFTY's 65
  ...
  for (const k of dayKeys) {
    ...
    for (let i = 0; i < bars.length; i++) {
      ...
      const sig = engine.getSignal(upTo, { prefix: PREFIX, silent: true });
...
function _renderResults(res, from, to, trades, stats, meta) {
  ...
  const g   = getGuardConfig();
  const c   = engine.getConfig(PREFIX);
  ...
  const html = renderBacktestResults({
    ...
    summary: stats,
    trades,
    activePage: "bnEmaRsiStV2Backtest",
    extraTradeColumns: [
      { key: "rsi",     label: "RSI" },
    ...
    notes:
      ...
      `The only other exits are the <b>opposite signal</b> and the hard square-off at ${_fmtMins(g.eodExitMin)} IST (<code>BN_EMA_RSI_ST_V2_EOD_EXIT_TIME</code>). ` +
  ...
  if (!jobId) {
    ...
    (async () => {
      try {
        ...
        backtestJobs.updateProgress(id, { phase: `Fetching ${res5}-min ${_u().label} candles (+${g.warmupDays}d warm-up runway)…`, pct: 20 });
        ...
        const need = engine.warmupBars(engine.getConfig(PREFIX));
        if (candles.length < need + 10) {
          backtestJobs.failJob(id,
            `Only ${candles.length} candle(s) for ${fetchFrom} → ${to}. The BN_EMA_RSI_ST_V2 engine refuses below ${need} candles (more than one session), so this range cannot produce a single trade. Widen the range or raise BN_EMA_RSI_ST_V2_WARMUP_DAYS (currently ${g.warmupDays}).`);
          return;
        }

        backtestJobs.updateProgress(id, { phase: `Running ${LABEL} backtest (${candles.length.toLocaleString()} candles)…`, pct: 70 });
        const result = runBnEmaRsiStV2Backtest(candles, activeFromTs);
