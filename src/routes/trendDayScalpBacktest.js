...
function runTrendDayScalpBacktest(intraday) {
  ...
  const MAX_DAY_LOSS = parseFloat(process.env.TDS_MAX_DAILY_LOSS   || "1500");
  const PROFIT_LOCK  = parseFloat(process.env.TDS_DAILY_PROFIT_LOCK || "1500");
...
function renderErrorPage(msg, from, to) {
  return `<!DOCTYPE html><html><head><meta charset="UTF-8"/><meta name="viewport" content="width=device-width,initial-scale=1"/>${faviconLink()}<title>Trend Day Scalp Backtest Error</title>
