...
async function squareOff(exitPrice, reason) {
  ...
  if (!result.success) {
    if (result.reason !== "duplicate_guard") {
      log(`🚨 [PA-LIVE] EXIT ORDER FAILED — check Fyers dashboard!`);
    }
  ...
  if (state.position) {
    ...
    if (pos.target) {
      if (pos.side === "CE" && price >= pos.target) {
        squareOff(pos.target, `Target hit ₹${pos.target.toFixed(0)}`).catch(e => console.error(`🚨 [PA-LIVE] squareOff error: ${e.message}`));
        return;
...
router.get("/start", async (req, res) => {
  ...
  if (!check.allowed) {
    return res.status(409).send(errorPage("Cannot Start", check.reason, "/pa-live/status", "\u2190 Back"));
  }
  ...
  if (process.env.PA_ENABLED !== "true") {
    return res.status(400).send(errorPage("PA Disabled", "PA_ENABLED is not true. Enable it in Settings first.", "/settings", "Open Settings"));
  }

  const holiday = await isTradingAllowed();
  if (!holiday.allowed) {
    return res.status(400).send(errorPage("Trading Not Allowed", holiday.reason, "/pa-live/status", "\u2190 Back"));
  }

  if (!isStartAllowed()) {
    return res.status(400).send(errorPage("Session Closed", "Past stop time \u2014 cannot start today.", "/pa-live/status", "\u2190 Back"));
  }
