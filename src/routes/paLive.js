 ...
 *
 * Routes:
 *   GET /pa-live/start   → Start price action live trading
 *   GET /pa-live/stop    → Stop & square off
 *   GET /pa-live/status  → Live status page
 *   GET /pa-live/exit    → Manual exit current position
 * ─────────────────────────────────────────────────────────────────────────────
 */
...
const SL_FILE  = path.join(DATA_DIR, "pa_live_trades.json");
    ...
    _optionPollTimer = setTimeout(async () => {
      ...
      if (ltp && state.position) {
        ...
        if (!state.position.optionEntryLtp) {
          ...
          log(`📌 [PA-LIVE] Option entry LTP: ₹${ltp}`);
          placePAHardSL();
        }
      } else if (!ltp) {
        // ── LTP staleness alert ──
        ...
        if (state.optionLtpUpdatedAt && (Date.now() - state.optionLtpUpdatedAt) > _staleThreshold) {
          if (!state._ltpStaleLogged) {
            log(`⚠️ [PA-LIVE] Option LTP STALE — no update for ${Math.round((Date.now() - state.optionLtpUpdatedAt) / 1000)}s. P&L display may be inaccurate.`);
...
let _paHardSLOrderId = null;
let _paHardSLSymbol  = null;   // contract the resting SL-M belongs to (see the stacking guard below)

function isPAHardSLEnabled() {
  return process.env.HARD_SL_ENABLED === "true" && instrumentConfig.INSTRUMENT !== "NIFTY_FUTURES";
}

async function placePAHardSL() {
  if (!isPAHardSLEnabled() || !state.position) return;
  ...
  if (_paHardSLOrderId) {
    if (_paHardSLSymbol === pos.symbol) {
      log(`⚠️ [PA HARD SL] Not placing a second SL-M — ${_paHardSLOrderId} may still be resting on ${pos.symbol}.`);
      return;
    }
    log(`🚨 [PA HARD SL] SL-M ${_paHardSLOrderId} on ${_paHardSLSymbol} was never confirmed cancelled and is no longer tracked — cancel it on the Fyers dashboard.`);
  }
...
async function updatePAHardSL(newSpotSL) {
  if (!isPAHardSLEnabled() || !_paHardSLOrderId || !state.position) return;
  ...
  try {
    ...
    if (result.success) {
      log(`🔄 [PA HARD SL] Modified trigger → ₹${newTrigger}`);
    } else {
      log(`⚠️ [PA HARD SL] Modify failed: ${JSON.stringify(result.raw)}`);
    }
  } catch (err) {
    log(`❌ [PA HARD SL] Modify exception: ${err.message}`);
  }
}

async function cancelPAHardSL() {
  ...
  const _clear = () => { _paHardSLOrderId = null; _paHardSLSymbol = null; };
  ...
  try {
    ...
    if (result.success) {
      _clear();
      log(`🗑️ [PA HARD SL] Cancelled SL-M order ${orderId}`);
    } else {
      log(`⚠️ [PA HARD SL] Cancel FAILED — keeping the id so a retry can cancel it. VERIFY SL-M ${orderId} is not still resting: ${JSON.stringify(result.raw)}`);
    }
  } catch (err) {
    log(`⚠️ [PA HARD SL] Cancel EXCEPTION — keeping the id so a retry can cancel it. VERIFY SL-M ${orderId} is not still resting: ${err.message}`);
  }
}
...
function verifyOrderFill(orderId, label) {
  ...
  setTimeout(async () => {
    try {
      ...
      if (!order) {
        log(`⚠️ [PA-LIVE] Order ${orderId} not found in order book (${label})`);
        return;
      }
      const status = (order.status || 0);
      // Fyers status: 2=TRADED/FILLED, 5=REJECTED, 1=PENDING, 6=CANCELLED
      if (status === 2) {
        log(`✅ [PA-LIVE] Order VERIFIED filled — ${orderId} (${label})`);
      } else if (status === 5) {
        log(`🚨 [PA-LIVE] Order REJECTED — ${orderId} (${label}) | ${order.message || "unknown"}`);
        sendTelegram(`🚨 PA Order REJECTED: ${label} | ${order.message || "unknown"}`).catch(() => {});
      } else {
        log(`⚠️ [PA-LIVE] Order status=${status} — ${orderId} (${label})`);
      }
    } catch (err) {
      log(`⚠️ [PA-LIVE] Order verification failed: ${err.message}`);
    }
  }, 3000);
}
...
async function placeOrder(fyersSymbol, side, qty) {
  ...
  try {
    const result = await fyersBroker.placeMarketOrder(
      fyersSymbol, side, qty, "PA_LIVE",
      { isFutures: instrumentConfig.INSTRUMENT === "NIFTY_FUTURES" }
    );
    if (result.success) {
      log(`✅ [PA-LIVE] Fyers order filled — ${sideLabel} ${qty} × ${fyersSymbol} | OrderID: ${result.orderId}`);
      verifyOrderFill(result.orderId, `${sideLabel} ${qty} × ${fyersSymbol}`);
    } else {
      log(`❌ [PA-LIVE] Fyers order FAILED — ${JSON.stringify(result.raw)}`);
    }
    return result;
  } catch (err) {
    log(`❌ [PA-LIVE] Order exception: ${err.message}`);
...
async function squareOff(exitPrice, reason) {
  if (_squareOffInFlight) {
    log(`⚠️ [PA-LIVE] squareOff already in progress — ignoring`);
    return;
  }
  ...
  if (_paHardSLOrderId) {
    await cancelPAHardSL();
    if (_paHardSLOrderId) {
      const _orphanSL = _paHardSLOrderId;
      _paHardSLOrderId = null; _paHardSLSymbol = null;
      log(`🚨 [PA HARD SL] SL-M ${_orphanSL} could NOT be cancelled and the position is now closed — cancel it on the Fyers dashboard NOW or it will short you.`);
      sendTelegram(`🚨 PA — SL-M ${_orphanSL} may still be resting after the position closed. Cancel it on Fyers NOW.`).catch(() => {});
    }
  }
  clearPAPosition();  // remove persisted state — position is closed
  ...
  if (reason.includes("SL")) {
    ...
    const pauseCandles = state._consecSLs >= 2
      ...
      : _PA_PAUSE_CANDLES;
    state._slPauseUntil = Date.now() + (pauseCandles * PA_RES * 60 * 1000);
    const escalateNote = state._consecSLs >= 2 ? ` (${state._consecSLs} consecutive SLs → ${pauseCandles} candles)` : "";
    log(`⏸️ [PA-LIVE] SL pause — ${pauseCandles} candles${escalateNote}`);
  } else if (netPnl > 0) {
    state._consecSLs = 0;
  }
...
function onTick(tick) {
  ...
  if (!state.currentBar || state.barStartTime !== bucketMs) {
    if (state.currentBar) {
      ...
      onCandleClose(state.currentBar).catch(e => console.error(`🚨 [PA-LIVE] onCandleClose error: ${e.message}`));
    }
  ...
  if (state.position) {
    ...
    if (pos.side === "CE" && price <= pos.stopLoss) {
      ...
      squareOff(pos.stopLoss, _isTrail ? `${_src} Trail SL hit` : `${_src} SL hit`).catch(e => console.error(`🚨 [PA-LIVE] squareOff error: ${e.message}`));
      return;
    }
    ...
    if (getISTMinutes() >= _STOP_MINS - 10) {
      squareOff(price, "EOD square-off").catch(e => console.error(`🚨 [PA-LIVE] squareOff error: ${e.message}`));
      return;
    }
  }
    ...
    console.error(`🚨 [PA-LIVE] onTick crash caught: ${err.message}`, err.stack);
  }
}
...
async function onCandleClose(bar) {
  ...
  if (state.position) {
    ...
    if (window.length >= 15) {
      ...
      if (trailResult.sl !== state.position.stopLoss) {
        log(`📐 [PA-LIVE] Trail SL (${trailResult.source}): ₹${state.position.stopLoss} → ₹${trailResult.sl}`);
  ...
  // Entry
  if (!isMarketHours()) {
    if (!state._omhLogged) { log(`⏭️ [PA-LIVE] Waiting for market open — entries start at the market-hours window`); state._omhLogged = true; }
    return;
  }
...
async function resolveAndEnter(side, spot, result) {
  ...
  try {
    ...
    if (process.env.PA_ENABLED !== "true") {
      log(`⚠️ [PA-LIVE] PA_ENABLED is not true — skipping entry`);
      return;
    }
    ...
    log(`⚠️ [PA-LIVE] Entry failed: ${err.message}`);
  } finally {
...
async function preloadHistory() {
  try {
    ...
    if (candles && candles.length > 0) {
      ...
      log(`📦 [PA-LIVE] Pre-loaded ${state.candles.length} × ${PA_RES}-min candles (strategy ready!)`);
      ...
      if (todayC.length > 0 && yesterdayC.length > 0) {
        ...
        if (Math.abs(gapPts) >= GAP_THRESHOLD) {
          ...
          log(`🔔 [PA-LIVE] GAP ${dir} detected: ${Math.abs(gapPts).toFixed(0)} pts`);
          sendTelegram(`🔔 [PA-LIVE] GAP ${dir}: ${Math.abs(gapPts).toFixed(0)} pts`).catch(() => {});
        }
      }
    } else {
      log(`⚠️ [PA-LIVE] No historical candles found — will build from live ticks`);
    }
  } catch (err) {
    log(`⚠️ [PA-LIVE] Pre-load failed: ${err.message}`);
  }
  ...
  try {
    ...
    if (dailyCandles && dailyCandles.length >= 2) {
      ...
      if (pastDays.length >= 1) {
        ...
        log(`📊 [PA-LIVE] Prev day OHLC: H=${prev.high} L=${prev.low} C=${prev.close}`);
      }
      ...
      if (_prevDayOHLC) {
        log(`📊 [PA-LIVE] Prev Day: H=${_prevDayOHLC.high} L=${_prevDayOHLC.low} C=${_prevDayOHLC.close}`);
      }
    } else {
      log(`⚠️ [PA-LIVE] Not enough daily candles for prev day data`);
    }
  } catch (err) {
    log(`⚠️ [PA-LIVE] Prev day data fetch failed: ${err.message}`);
  }
}

// ── EOD Backup Timer — force exit at 3:25 PM IST if tick-based exit missed ──
let _paEodBackupTimer = null;

function schedulePAEODBackup() {
  clearPAEODBackup();
  ...
  _paEodBackupTimer = setTimeout(() => {
    if (!state.running || !state.position) return;
    const exitPrice = state.lastTickPrice || (state.currentBar ? state.currentBar.close : 0);
    log(`🚨 [PA-LIVE] EOD BACKUP TIMER — force exit at 3:25 PM IST`);
    squareOff(exitPrice, "EOD backup timer (3:25 PM)").catch(e => log(`❌ [PA-LIVE] EOD backup exit error: ${e.message}`));
  }, msUntil);
  log(`⏰ [PA-LIVE] EOD backup timer set — force exit in ${Math.round(msUntil / 60000)} min`);
}

function clearPAEODBackup() {
  if (_paEodBackupTimer) { clearTimeout(_paEodBackupTimer); _paEodBackupTimer = null; }
}
...
function scheduleAutoStop(stopFn) {
  ...
  _autoStopTimer = setTimeout(() => {
    if (!state.running) return;
    stopFn("⏰ [PA-LIVE] Auto-stop reached");
  }, ms);
  log(`⏰ [PA-LIVE] Auto-stop in ${Math.round(ms / 60000)} min`);
}

// errorPage imported from sharedNav (shared across all route files)
function _errorPage(title, message, linkHref, linkText) {
  return errorPage(title, message, linkHref, linkText, 'paLive');
}
...
router.get("/start", async (req, res) => {
  ...
  if (process.env.PA_ENABLED !== "true") {
    return res.status(400).send(_errorPage("PA Disabled", "PA_ENABLED is not true. Enable it in Settings first.", "/settings", "Open Settings"));
  }

  const holiday = await isTradingAllowed();
  if (!holiday.allowed) {
    return res.status(400).send(_errorPage("Trading Not Allowed", holiday.reason, "/pa-live/status", "\u2190 Back"));
  }

  if (!isStartAllowed()) {
    return res.status(400).send(_errorPage("Session Closed", "Past stop time \u2014 cannot start today.", "/pa-live/status", "\u2190 Back"));
  }
  ...
  try {
    ...
    if (openPos.length > 0) {
      ...
      log(`⚠️ [PA-LIVE] Broker has open positions: ${symbols}`);
      ...
      log(`✅ [PA-LIVE] No orphaned positions on Fyers — clean start`);
    }
  } catch (err) {
    log(`⚠️ [PA-LIVE] Position reconciliation failed: ${err.message}`);
  }

  // Socket: piggyback or start own
  if (socketManager.isRunning()) {
    socketManager.addCallback(CALLBACK_ID, onTick, log);
    log("📡 [PA-LIVE] Piggybacking on existing WebSocket");
  } else {
    socketManager.start(NIFTY_INDEX_SYMBOL, () => {}, log);
    socketManager.addCallback(CALLBACK_ID, onTick, log);
    log("📡 [PA-LIVE] Started WebSocket");
  }
...
async function stopSession() {
  ...
  if (state.position) {
    try { await awaitExit(squareOff(state.lastTickPrice || state.position.entryPrice, "Session stopped"), "PA-LIVE"); }
      ...
      console.error(`🚨 [PA-LIVE] ${e.message}`);
      sendTelegram(`🚨 PA EXIT NOT CONFIRMED on session stop — ${e.message}`).catch(() => {});
    }
  }
...
router.get("/exit", async (req, res) => {
  if (state.position) {
    try { await awaitExit(squareOff(state.lastTickPrice || state.position.entryPrice, "Manual exit"), "PA-LIVE"); }
    catch (e) {
...
router.post("/manualEntry", async (req, res) => {
  if (!state.running) return res.status(400).json({ success: false, error: "Price Action live is not running." });
...
router.get("/status/data", (req, res) => {
  ...
  res.json({
    ...
    console.error("[pa-live/status/data] Error:", err.message);
...
/* Toast */
.pa-toast{position:fixed;bottom:32px;left:50%;transform:translateX(-50%);background:#0d1320;padding:12px 24px;border-radius:10px;font-size:0.85rem;font-weight:700;z-index:9999;box-shadow:0 4px 24px rgba(0,0,0,0.6);letter-spacing:0.5px;animation:fadeIn 0.2s ease;}
...
<div class="app-shell">
${buildSidebar('paLive', liveActive, state.running, {
  showExitBtn: !!pos,
...
<div class="page">
  <div class="page-header">
    <div class="page-status-row">
      ...
      <span class="page-status-text ${state.running ? 'running' : ''}">${state.running ? 'PRICE ACTION LIVE ACTIVE' : 'STOPPED'}</span>
    </div>
  </div>
  ...
  <div class="stat-grid">
    ...
    <div class="sc" style="border-top:2px solid ${state._dailyLossHit ? '#ef4444' : '#10b981'};" id="ax-sc-dloss">
      ...
      <div class="sc-val" style="color:${state._dailyLossHit ? '#ef4444' : '#fff'};">${inr(-_PA_MAX_LOSS)}</div>
...
async function scHandleExit(btn) {
  ...
  try {
    var res = await secretFetch('/pa-live/exit');
...
async function scHandleStart(btn) {
  ...
  try {
    var res = await secretFetch('/pa-live/start');
    ...
    scToast('\uD83D\uDD34 PA live trading started!', '#10b981');
    setTimeout(function(){ location.reload(); }, 1200);
  } catch(e) {
    scToast('\u274c ' + e.message, '#ef4444');
    if (btn) { btn.textContent = '\u25b6 Start'; btn.disabled = false; }
...
async function scHandleStop(btn) {
  ...
  try {
    var res = await secretFetch('/pa-live/stop');
    if (!res) { if (btn) { btn.textContent = '\u25a0 Stop'; btn.disabled = false; } return; }
    var data = await res.json();
    scToast('\u23f9 PA live trading stopped.', '#ef4444');
    setTimeout(function(){ location.reload(); }, 1200);
  } catch(e) {
    scToast('\u274c ' + e.message, '#ef4444');
...
function showSCModal(t){
  ...
  var exitHtml='<div style="background:#0c0608;border:1px solid #3a0d12;border-radius:10px;padding:12px 14px;margin-bottom:10px;">'
    ...
    +(function(){
        ...
        var peakGain=(t.eOpt!=null)?parseFloat((t.peakOpt-t.eOpt).toFixed(2)):null;
        var giveback=(t.xOpt!=null)?parseFloat((t.peakOpt-t.xOpt).toFixed(2)):null;
        var sub=(peakGain!=null?'+'+peakGain+' pts peak':'peak')+(giveback!=null?' \u00b7 gave back '+giveback:'');
...
(function() {
  ...
  function fetchAndUpdate() {
    fetch('/pa-live/status/data', { cache: 'no-store' }).then(function(r) { return r.json(); }).then(function(d) {
...
${optionChart.optionChartScript({ dataUrl: '/pa-live/status/chart-data', id: 'pal-opt-chart' })}
  ...
  return res.send(html);
    ...
    console.error("[pa-live/status] Error:", err.message, err.stack);
    return res.status(500).send(`<pre style="color:red;padding:32px;font-family:monospace;">Price Action Live Status Error: ${err.message}\n\n${err.stack}</pre>`);
  }
});
