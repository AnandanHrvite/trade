  ...
  if (netPnl < 0) {
    ...
    if (tradeState._consecutiveLosses >= 3) {
      if (TRADE_RES >= 15) {
        ...
        const pauseMs = 2 * TRADE_RES * 60 * 1000;
        ...
        log(`⚠️ [LIVE] 3 consecutive losses — entries PAUSED for ${TRADE_RES * 2} min (resume ~${resumeTime})`);
        tradeState._consecutiveLosses = 0;
      }
    }
  } else {
...
<div class="page">
  ...
  <div>
    ...
    ${tradeState.sessionTrades.length === 0
      ...
      : `<div style="border:1px solid #1a2236;border-radius:12px;overflow:hidden;margin-bottom:10px;">
