  ...
  fetchOptionLtp(symbol).then(ltp => {
    ...
    if (tradeState.position) {
      ...
      if (!tradeState.position.optionEntryLtp) {
        ...
        log(`📌 Option entry LTP captured: ₹${ltp} (REST, first poll)`);
      }
    }
  });
...
function parseOptionDetails(symbol) {
  ...
  try {
    ...
    // Format B: YY + 3-letter month + 2-digit day (legacy monthly)
...
async function onCandleClose(candle) {
  ...
  if (tradeState.position && stopLoss != null) {
    ...
    if (tighten && oldSL !== stopLoss) {
      ...
      log(`🔄 [LIVE] SAR trail: ₹${oldSL} → ₹${stopLoss} (${pos.side === "CE" ? "↑" : "↓"} tightened)`);
    }
  }
    ...
    if (!isEntryCandle) {
      ...
      if (prevMid !== null) {
        if (tradeState.position.side === "CE" && candle.low < prevMid) {
          log(`🛑 [LIVE] 50% rule CE — candle low ₹${candle.low} < entry prev mid ₹${prevMid}`);
          await squareOff(prevMid, `50% rule — low ₹${candle.low} < prev mid ₹${prevMid}`);
          return;
        }
        if (tradeState.position.side === "PE" && candle.high > prevMid) {
          log(`🛑 [LIVE] 50% rule PE — candle high ₹${candle.high} > entry prev mid ₹${prevMid}`);
  ...
  if (tradeState.position && tradeState.position.stopLoss != null) {
    ...
    if (tradeState.position.side === "CE" && candle.close < sl) {
      log(`🚨 [LIVE] SL hit on candle close — ${candle.close} < SL ${sl}`);
  ...
  if (!tradeState.position && !tradeState._entryPending && isMarketHours() && (signal === "BUY_CE" || signal === "BUY_PE")) {
    ...
    try {
      ...
      log(`📝 [LIVE] ${entryLabel} @ SPOT ₹${candle.close} | SL: ₹${stopLoss} | OrderID: ${result.orderId || "?"}`);
      // ── Telegram notification ───────────────────────────────────────────────
      notifyEntry({
        mode:           "LIVE",
        side,
...
function onSpotTick(tick) {
  ...
  if (tradeState.position && tradeState.position.stopLoss !== null) {
    ...
    const TRAIL_ACTIVATE = _TRAIL_ACTIVATE_PTS;
    ...
    if (pos.side === "CE") {
      ...
      if (moveInFavour >= TRAIL_ACTIVATE) {
        ...
        if (trailSL > pos.stopLoss) {
          log(`📈 [LIVE] Trail CE [T${moveInFavour<_TRAIL_T1_UPTO?1:moveInFavour<_TRAIL_T2_UPTO?2:3} gap=${dynamicGap}pt]: best=₹${pos.bestPrice} (+${moveInFavour.toFixed(0)}pt) → SL ₹${pos.stopLoss} → ₹${trailSL}`);
      ...
      if (moveInFavour >= TRAIL_ACTIVATE) {
        ...
        const trailSL    = parseFloat((pos.bestPrice + dynamicGap).toFixed(2));
        if (trailSL < pos.stopLoss) {
          log(`📉 [LIVE] Trail PE [T${moveInFavour<_TRAIL_T1_UPTO?1:moveInFavour<_TRAIL_T2_UPTO?2:3} gap=${dynamicGap}pt]: best=₹${pos.bestPrice} (+${moveInFavour.toFixed(0)}pt) → SL ₹${pos.stopLoss} → ₹${trailSL}`);
          pos.stopLoss = trailSL;
        }
    ...
    <div style="background:#0a1f0a;border:1px solid #065f46;border-radius:12px;padding:20px 24px;">
      ...
      <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(130px,1fr));gap:10px;margin-bottom:12px;">
        ...
        <div id="ajax-trail-card" style="background:#071a12;border:1px solid ${trailActive && trailProfit > 0 ? "#8b5cf6" : "#134e35"};border-radius:8px;padding:12px 14px;">
          ...
          <div id="ajax-trail-status" style="font-size:0.88rem;font-weight:700;color:${trailActive && trailProfit > 0 ? "#8b5cf6" : "#4a6080"};">${trailActive && trailProfit > 0 ? "🔒 ACTIVE" : "⏳ Waiting"}</div>
...
<head>
  ...
  <title>Palani Andawar thunai — Live Trade</title>
  ...
  <style>
    *{box-sizing:border-box;margin:0;padding:0;}
    ...
    nav .brand{font-size:1rem;font-weight:700;color:#fff;}nav .brand span{color:#ef4444;}
...
<nav style="display:flex;align-items:center;justify-content:space-between;padding:10px 24px;border-bottom:1px solid #1a2236;background:#0d1320;position:sticky;top:0;z-index:100;gap:12px;">
  ...
  <div style="display:flex;gap:4px;flex-wrap:wrap;align-items:center;">
    ...
    ${tradeState.running
      ...
      : `<button onclick="ltHandleStart(this)" style="cursor:pointer;font-family:inherit;font-size:0.76rem;font-weight:700;color:#10b981;background:#071a12;border:1px solid #065f46;padding:6px 14px;border-radius:6px;white-space:nowrap;">▶ Start</button>`}
  </div>
</nav>
<div class="page">
  <div style="margin-bottom:28px;">
    <div style="display:flex;align-items:center;gap:12px;margin-bottom:8px;">
      ${tradeState.running
        ? `<span style="width:10px;height:10px;border-radius:50%;background:#ef4444;display:inline-block;animation:pulse 1.5s infinite;"></span><span style="font-size:0.8rem;font-weight:700;color:#ef4444;text-transform:uppercase;letter-spacing:1px;">⚡ LIVE TRADING ACTIVE</span>`
    ...
    <h1 style="font-size:1.5rem;font-weight:700;color:#fff;letter-spacing:-0.5px;">Live Trade</h1>
    <p style="font-size:0.85rem;color:#4a6080;margin-top:4px;">Strategy: <strong style="color:#3b82f6;">${ACTIVE} — ${strategy.NAME}</strong> &nbsp;·&nbsp; <strong style="color:#c8d8f0;">${getTradeResolution()}-min candles</strong> &nbsp;·&nbsp; ${tradeState.running ? 'Auto-refreshes every 2s' : 'Not refreshing — trading stopped'}</p>
  </div>

  ${expiryBanner}
  <div style="display:flex;gap:10px;flex-wrap:wrap;margin-bottom:20px;">${fyersBadge}${zerodhaBadge}</div>
