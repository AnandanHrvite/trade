...
function onTick(tick) {
  ...
  if (state.position) {
    ...
    if (_SCALP_MAX_TARGET > 0 && _tickPnl(price) >= _SCALP_MAX_TARGET) {
      simulateSell(price, `Max target ₹${_SCALP_MAX_TARGET}`, price);
      return;
    }
  ...
  if (state.position) {
    ...
    if (_SCALP_MIN_TARGET > 0 && _candlePnl(bar.close) >= _SCALP_MIN_TARGET) {
      if (window.length >= 15 && scalpStrategy.isPSARFlip(window, state.position.side)) {
        simulateSell(bar.close, `Target ₹${Math.round(_candlePnl(bar.close))} + PSAR flip`, bar.close);
        return;
      }
    }
    ...
    if (window.length >= 15 && scalpStrategy.isPSARFlip(window, state.position.side)) {
      ...
      log(`📊 [SCALP-PAPER] PSAR flip but PnL ₹${Math.round(curPnl)} < target ₹${_SCALP_MIN_TARGET} — holding`);
    }
