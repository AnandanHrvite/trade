...
router.get("/", async (req, res) => {
  ...
  // An explicit ?resolution= still wins (that is what "Run Again" and the unified
  // dashboard send); with no query param the run uses BB_RSI's own candle size.
  const resolution = req.query.resolution || String(bbRsiStrategy.resolutionMin());
    ...
    <div style="background:#0d1320;border:1px solid #1d3b6e;border-radius:16px;padding:24px 28px;max-width:680px;width:100%;max-height:90vh;overflow-y:auto;box-shadow:0 24px 80px rgba(0,0,0,0.9);position:relative;">
      <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:18px;">
        <div>
          ...
          <span style="font-size:0.65rem;color:var(--muted-1,#8ba1c2);margin-left:10px;">\u26a1 BB_RSI Backtest — Full Details</span>
        </div>
...
${aiExportScriptTag({ mode: "BB_RSI", strategyName: bbRsiStrategy.NAME, from, to, optionSim: !!(s && s.optionSim), fullCount: tradesData.length })}
