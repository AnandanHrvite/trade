/**
 * BN_PIVOT_RSI_ST LIVE (HARNESS) — /bn-pivot-rsi-st-live
 * ─────────────────────────────────────────────────────────────────────────────
 * Runs BN_PIVOT_RSI_ST LIVE by wrapping its PAPER route with the live harness:
 *   1. Install harness (registers notify entry/exit hooks → real Zerodha orders)
 *   2. Trigger /bn-pivot-rsi-st-paper/start programmatically (paper runs unchanged)
 ...
 * whatever bnPivotRsiStPaper says it is (single source of truth).
 *
 * UNDERLYING: NIFTY BANK (NSE:NIFTYBANK-INDEX), not NIFTY 50. Same rules as
 * RSI_PIVOT_ST — only the instrument differs: a 100-point strike grid and
 * MONTHLY-only expiries (NSE withdrew BANKNIFTY weeklies in Nov-2024). This
 * harness never resolves a symbol itself; the paper route does, through
 * instrumentConfig with `underlying: "BANKNIFTY"`.
 *
 ...
 *   BN_PIVOT_RSI_ST_LIVE_ENABLED=true   (per-strategy live enable, default false)
 *   LIVE_HARNESS_DRY_RUN=false       (global kill-switch, default true)
 *   BN_PIVOT_RSI_ST_LIVE_DRY_RUN!=true  (per-strategy dry-run override)
...
const bnPaperRoute = require("./bnPivotRsiStPaper");
...
/** "MONTHLY expiry only" / "weekly + monthly" — whichever the config says now. */
...
function _invokePaperRoute(method, urlPath) {
  return new Promise((resolve, reject) => {
    ...
    const stack = bnPaperRoute.stack || [];
    let i = 0;
    function next(err) {
...
router.get("/status/data", async (req, res) => {
  const cfg = liveHarness.getConfig("BN_PIVOT_RSI_ST-LIVE");
  let paperData = {};
  try {
  ...
  res.json({
    ...paperData,
    installed:    liveHarness.isInstalled("BN_PIVOT_RSI_ST-LIVE"),
    config:       cfg,
    recentEvents: liveHarness.getRecentEvents(50, "BN_PIVOT_RSI_ST-LIVE"),
  });
});

router.get("/start", async (req, res) => {
  if (liveHarness.isInstalled("BN_PIVOT_RSI_ST-LIVE")) {
    return res.status(409).json({ success: false, error: "BN_PIVOT_RSI_ST-LIVE harness is already running. Stop it first." });
  }

  if (String(process.env.BN_PIVOT_RSI_ST_MODE_ENABLED || "true").toLowerCase() !== "true") {
    return res.status(403).json({ success: false, error: "BN Pivot RSI ST mode is disabled. Enable it in Settings first." });
  }

  const dryRun = liveDryRun.isDryRun("BN_PIVOT_RSI_ST");

  if (!liveDryRun.isDryRun() && String(process.env.BN_PIVOT_RSI_ST_LIVE_ENABLED || "false").toLowerCase() !== "true") {
    return res.status(403).json({ success: false, error: "Live trading disabled. Set BN_PIVOT_RSI_ST_LIVE_ENABLED=true to place real orders." });
  }
  ...
  try {
    installed = liveHarness.installHarness({
      ...
      // in tradeLogger's FILE_BY_MODE and DAILY_PREFIX_BY_MODE — the string must
      // match CHARACTER FOR CHARACTER, because nothing normalises it and an
      // unregistered key makes appendTradeLog throw into its own catch, dropping
      // every live trade from both JSONL logs while the run still looks healthy.
  ...
  try {
    ...
    if (startResp.status >= 400 && startResp.status !== 302) {
      ...
      return res.status(startResp.status).json({
        success: false,
        error:   `bnPivotRsiStPaper /start failed: ${JSON.stringify(startResp.body).slice(0, 300)}`,
      });
    }
    return res.json({
      success: true,
      ...
      message: installed.dryRun
        ? "BN_PIVOT_RSI_ST-LIVE harness started in DRY-RUN. Decisions match paper, no real orders placed. Watch /bn-pivot-rsi-st-live."
        : "BN_PIVOT_RSI_ST-LIVE harness started — real Zerodha orders WILL be placed.",
...
router.get("/stop", async (req, res) => {
  ...
  try {
    ...
    liveHarness.uninstallHarness("BN_PIVOT_RSI_ST-LIVE");
    return res.json({ success: true, message: "BN_PIVOT_RSI_ST-LIVE harness stopped + paper session ended.", paperStopResp: stopResp });
  } catch (err) {
    try { liveHarness.uninstallHarness("BN_PIVOT_RSI_ST-LIVE"); } catch (_) {}
    return res.status(500).json({ success: false, error: err.message });
  }
});
...
router.get("/", (req, res) => {
  const cfg = liveHarness.getConfig("BN_PIVOT_RSI_ST-LIVE");
  const installed = liveHarness.isInstalled("BN_PIVOT_RSI_ST-LIVE");
  const dryRunCurrent = liveDryRun.isDryRun("BN_PIVOT_RSI_ST");
  const scfg = bnPivotStrategy.getConfig();
  const html = `<!DOCTYPE html>
<html><head>
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width,initial-scale=1">
<title>BN Pivot RSI ST LIVE (Harness) — Real orders via Paper engine</title>
${faviconLink()}
<style>
${sidebarCSS()}
${modalCSS()}
...
<body>
${buildSidebar('bnPivotRsiStLive', false)}
<div class="main">
  <h1>● BN PIVOT RSI ST LIVE — via Paper Harness</h1>
  ...
  <div class="card">
    <div class="row">
      ...
      <div style="flex:2;min-width:220px;">
        ...
        <div class="val" style="font-size:0.8rem;">${_u().label} (BANKNIFTY) ${scfg.resolutionMins}m — <code>${_u().spot}</code>, strikes every ${bnPivotStrategy.strikeStep()} pts, lot ${instrumentConfig.getLotQty(UNDERLYING)}, <b>${_expiryLabel()}</b>. Yesterday's H/L/C fixes today's Standard Pivots. A candle that CROSSES and CLOSES above R1 with RSI&gt;${scfg.rsiCeMin} buys a CE; below S1 with RSI&lt;${scfg.rsiPeMax} buys a PE. Strike = ${scfg.strikeMode} at ${scfg.strikePct}% of spot, rounded to the nearest ${bnPivotStrategy.strikeStep()}-pt BANKNIFTY strike. SuperTrend(${scfg.stPeriod},${scfg.stMultiplier}) stops <b>${scfg.stSides}</b>; the ${scfg.premiumStopPct}% premium floor applies to <b>${scfg.premiumStopSides}</b>. No profit target.${["CE","PE"].filter(x => bnPivotStrategy.isStoplessSide(x, scfg)).map(x => ` <b style="color:#f85149;">⚠️ ${x} trades currently have NO stop — only the EOD square-off can close them.</b>`).join("")}</div>
      </div>
    </div>
  ...
  <div class="card">
    ...
    <div class="row" id="bn-live-context"><div class="val">Loading…</div></div>
  </div>

  <div class="card">
    <div class="section-title">NIFTY BANK intraday — pivots, SuperTrend, bracket</div>
...
async function startSession() {
  ...
  try {
    const r = await secretFetch('/bn-pivot-rsi-st-live/start');
...
async function stopSession() {
  ...
  try {
    const r = await secretFetch('/bn-pivot-rsi-st-live/stop');
