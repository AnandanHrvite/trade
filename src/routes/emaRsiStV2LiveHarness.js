/**
 * EMA_RSI_ST_V2 LIVE (HARNESS) — /ema_rsi_st_v2-live-harness
 * ─────────────────────────────────────────────────────────────────────────────
 * Runs EMA_RSI_ST_V2 LIVE by wrapping EMA_RSI_ST_V2 PAPER with the live harness:
 *   1. Install harness (registers notify entry/exit hooks → real broker orders)
 *   2. Trigger /ema_rsi_st_v2-paper/start programmatically (paper code runs unchanged)
 ...
 * is whatever emaRsiStV2Paper says it is — single source of truth.
 *
 ...
 *
 * Toggles:
 *   UI_SHOW_EMA_RSI_ST_V2_LIVE_HARNESS  — show the menu item (Settings)
 *   LIVE_HARNESS_DRY_RUN        — when true (default), no real orders placed
 *   EMA_RSI_ST_V2_LIVE_ENABLED          — per-strategy live gate (default FALSE)
 *   EMA_RSI_ST_V2_LIVE_DRY_RUN          — hold EMA_RSI_ST_V2 in dry-run even when global is off
 * ─────────────────────────────────────────────────────────────────────────────
 */
...
const emaRsiStV2PaperRoute = require("./emaRsiStV2Paper");
const liveDryRun      = require("../utils/liveDryRun");
const { buildSidebar, sidebarCSS, faviconLink, modalCSS, modalJS } = require("../utils/sharedNav");

// ── Programmatic invoker for the emaRsiStV2Paper express router ──────────────────
function _invokePaperRoute(method, urlPath) {
  return new Promise((resolve, reject) => {
    ...
    const stack = emaRsiStV2PaperRoute.stack || [];
    let i = 0;
    function next(err) {
...
router.get("/status/data", (req, res) => {
  const cfg = liveHarness.getConfig("EMA_RSI_ST_V2-LIVE");
  res.json({
    installed:    liveHarness.isInstalled("EMA_RSI_ST_V2-LIVE"),
    config:       cfg,
    recentEvents: liveHarness.getRecentEvents(50, "EMA_RSI_ST_V2-LIVE"),
  });
});

router.get("/start", async (req, res) => {
  if (liveHarness.isInstalled("EMA_RSI_ST_V2-LIVE")) {
    return res.status(409).json({ success: false, error: "EMA_RSI_ST_V2-LIVE harness is already running. Stop it first." });
  }

  // Default DRY-RUN unless user explicitly set LIVE_HARNESS_DRY_RUN=false.
  const dryRun = liveDryRun.isDryRun("EMA_RSI_ST_V2");

  // Live-order gate (the documented double-gate): real orders require EMA_RSI_ST_V2_LIVE_ENABLED=true.
  ...
  if (!liveDryRun.isDryRun() && (process.env.EMA_RSI_ST_V2_LIVE_ENABLED || "false").toLowerCase() !== "true") {
    return res.status(403).json({ success: false, error: "Live trading disabled. Set EMA_RSI_ST_V2_LIVE_ENABLED=true to place real orders." });
  }
  ...
  try {
    installed = liveHarness.installHarness({
      mode:       "EMA_RSI_ST_V2-LIVE",
      modeTag:    "PAPER",       // emaRsiStV2Paper's mode field in notify payloads
      broker:     "zerodha",
      dryRun,
  ...
  // Trigger emaRsiStV2Paper /start — paper runs unchanged, harness intercepts its
  ...
  console.log(`🧪 [EMA_RSI_ST_V2-LIVE-HARNESS] confirmation candle: ${(process.env.EMA_RSI_ST_V2_CONFIRM_CANDLE_ENABLED || "true").toLowerCase() === "true" ? "ON (2-candle cross & close)" : "OFF (legacy intra-candle)"}`);
  try {
    const startResp = await _invokePaperRoute("GET", "/start");
    if (startResp.status >= 400 && startResp.status !== 302) {
      liveHarness.uninstallHarness("EMA_RSI_ST_V2-LIVE");
      return res.status(startResp.status).json({
        success: false,
        error:   `emaRsiStV2Paper /start failed: ${JSON.stringify(startResp.body).slice(0, 300)}`,
      });
    }
    return res.json({
      success: true,
      ...
      message: installed.dryRun
        ? "EMA_RSI_ST_V2-LIVE harness started in DRY-RUN. Decisions match paper, no real orders placed. Watch /ema_rsi_st_v2-live-harness."
        : "EMA_RSI_ST_V2-LIVE harness started — real Zerodha orders WILL be placed.",
...
router.get("/stop", async (req, res) => {
  ...
  try {
    ...
    liveHarness.uninstallHarness("EMA_RSI_ST_V2-LIVE");
    return res.json({ success: true, message: "EMA_RSI_ST_V2-LIVE harness stopped + paper session ended.", paperStopResp: stopResp });
  } catch (err) {
    try { liveHarness.uninstallHarness("EMA_RSI_ST_V2-LIVE"); } catch (_) {}
    return res.status(500).json({ success: false, error: err.message });
  }
});
...
router.get("/", (req, res) => {
  const cfg = liveHarness.getConfig("EMA_RSI_ST_V2-LIVE");
  const installed = liveHarness.isInstalled("EMA_RSI_ST_V2-LIVE");
  const dryRunCurrent = liveDryRun.isDryRun("EMA_RSI_ST_V2");
  const html = `<!DOCTYPE html>
<html><head>
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width,initial-scale=1">
<title>EMA_RSI_ST_V2 LIVE (Harness) — Real orders via Paper engine</title>
${faviconLink()}
<style>
${sidebarCSS()}
${modalCSS()}
...
<body>
${buildSidebar('emaRsiStV2LiveHarness', false)}
<div class="main">
  <h1>● EMA_RSI_ST_V2 LIVE — via Paper Harness</h1>

  ${dryRunCurrent
    ? '<div class="warn-soft"><strong>🧪 DRY-RUN mode</strong> — no real orders will be placed. Verify decisions match paper for at least one session, then set <code>LIVE_HARNESS_DRY_RUN=false</code> (and ensure <code>EMA_RSI_ST_V2_LIVE_DRY_RUN</code> is not true) in Settings to enable real orders.</div>'
...
async function refresh() {
  try {
    const r = await fetch('/ema_rsi_st_v2-live-harness/status/data');
...
async function startSession() {
  ...
  try {
    const r = await secretFetch('/ema_rsi_st_v2-live-harness/start');
...
async function stopSession() {
  ...
  try {
    const r = await secretFetch('/ema_rsi_st_v2-live-harness/stop');
