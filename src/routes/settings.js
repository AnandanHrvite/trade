...
const SETTINGS_SCHEMA = [
  ...
  },
  {
    ...
    fields: [
      ...
      { key: "ORB_SL_CANDLES", label: "Trailing Stop — Candle Count", type: "number", min: 1, max: 10, step: 1, effect: EFFECT.INSTANT, desc: "The ONLY stop: SL = swing of the last N closed candles (CE → lowest low, PE → highest high). Ratchets in your favour on each candle close — the same level is both the initial SL and the trailing SL. No premium SL, no profit target. 2 = last 2 candles.", default: "2" },
  ...
  <div class="main-content">
    <div class="top-bar">
      ...
      <div class="top-bar-btns" style="margin-left:auto;display:flex;gap:8px;flex-wrap:nowrap;overflow-x:auto;white-space:nowrap;">
        ...
        <button onclick="openResetDataModal()" title="Selectively reset data: paper trade history, skip history, cache, logs, or ticks — with an optional date range. Checking Paper with no date range also restores starting capital + wipes sessions for all 5 strategies (a running strategy is skipped)." style="padding:6px 14px;background:rgba(239,68,68,0.12);color:#f87171;border:1px solid rgba(239,68,68,0.35);border-radius:6px;font-size:0.75rem;font-weight:700;cursor:pointer;font-family:'IBM Plex Mono',monospace;letter-spacing:0.5px;">🧹 RESET DATA</button>
      </div>
    </div>
...
// ── Reset Data dialog ────────────────────────────────────────────────────────
// Opens a category-picker with an optional date range. Categories:
//   paper (daily trade JSONL), skip (daily skip JSONL), ticks — all date-scoped
//   by the range; cache & logs always clear fully. Checking "paper" with NO date
//   range also fans out to each strategy's canonical /reset route to wipe the
//   summary *_paper_trades.json and restore starting capital.
