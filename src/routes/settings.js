...
const SETTINGS_SCHEMA = [
  ...
  },
  {
    ...
    fields: [
      ...
      { key: "ORB_ENTRY_V2_ENABLED", label: "Entry Engine V2 (Confirmed Breakout)", type: "toggle", effect: EFFECT.INSTANT, desc: "ON (default) = 2026-07-09 redesign: strong breakout candle + NEXT-candle confirmation (HH/HC) + EMA20/50 & ADX trend regime + RSI + gap gate; buys the confirmation candle, one committed breakout attempt/day. OFF = legacy immediate-entry engine (buys the breakout candle's close; wick/VWAP filters only). Toggle OFF to A/B against the old baseline.", default: "true" },
      { key: "ORB_MIN_RANGE_PTS", label: "Min OR Width (pts)", type: "number", min: 10, max: 80, step: 5, effect: EFFECT.INSTANT, desc: "Skip the day when the opening range is too tight — noise filter (STEP 2)", default: "30" },
      { key: "ORB_MAX_RANGE_PTS", label: "Max OR Width (pts)", type: "number", min: 50, max: 300, step: 10, effect: EFFECT.INSTANT, desc: "Skip the day when the opening range is too wide — exhausted move / open already ran (STEP 2)", default: "80" },
      { key: "ORB_MIN_BODY", label: "Min Break Candle Body (pts)", type: "number", min: 3, max: 30, step: 1, effect: EFFECT.INSTANT, desc: "Breakout candle body must be at least this many points (STEP 4)", default: "15" },
      ...
      { key: "ORB_PREMIUM_MIN", label: "Min Option Premium (₹)", type: "number", min: 20, max: 500, step: 5, effect: EFFECT.INSTANT, desc: "STEP 8: skip entry if ATM LTP below this", default: "100" },
      { key: "ORB_PREMIUM_MAX", label: "Max Option Premium (₹)", type: "number", min: 100, max: 1000, step: 10, effect: EFFECT.INSTANT, desc: "STEP 8: skip entry if ATM LTP above this", default: "220" },
