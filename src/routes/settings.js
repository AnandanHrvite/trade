...
const SETTINGS_SCHEMA = [
  ...
  },
  {
    ...
    fields: [
      ...
      { key: "SCALP_SL_PTS", label: "Stop Loss (pts)", type: "number", min: 3, max: 50, step: 1, effect: EFFECT.SESSION, desc: "Fixed SL distance from entry", default: "12" },
      { key: "SCALP_TARGET_PTS", label: "Target (pts)", type: "number", min: 5, max: 50, step: 1, effect: EFFECT.SESSION, desc: "Fixed target distance from entry", default: "18" },
      { key: "SCALP_TRAIL_GAP", label: "Trail Gap (pts)", type: "number", min: 3, max: 30, step: 1, effect: EFFECT.SESSION, desc: "Trailing SL gap behind best price", default: "8" },
      { key: "SCALP_TRAIL_AFTER", label: "Trail After (pts)", type: "number", min: 3, max: 30, step: 1, effect: EFFECT.SESSION, desc: "Activate trail after this many pts profit", default: "10" },
      { key: "SCALP_TIME_STOP_CANDLES", label: "Time Stop (candles)", type: "number", min: 2, max: 20, step: 1, effect: EFFECT.SESSION, desc: "Exit if no target hit within N candles", default: "4" },
      { key: "SCALP_MAX_DAILY_TRADES", label: "Max Daily Trades", type: "number", min: 5, max: 100, step: 5, effect: EFFECT.SESSION, desc: "Maximum scalp entries per day", default: "30" },
      ...
      { key: "SCALP_MIN_BODY", label: "Min Body (pts)", type: "number", min: 2, max: 20, step: 1, effect: EFFECT.SESSION, desc: "Minimum candle body size", default: "5" },
      { key: "SCALP_ADX_ENABLED", label: "ADX Filter", type: "toggle", effect: EFFECT.SESSION, desc: "Enable ADX trend filter for scalp", default: "false" },
