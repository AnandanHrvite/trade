...
const SETTINGS_SCHEMA = [
  {
    ...
    fields: [
      ...
      { key: "SWING_STRONG_ONLY", label: "STRONG signals only", type: "toggle", effect: EFFECT.SESSION, desc: "Block MARGINAL signal entries (only steep-slope + committed-RSI signals fire). Reduces trade count, improves WR", default: "false" },
      ...
      { key: "SWING_PREV_CANDLE_TRAIL_ENABLED", label: "Prev-Candle Trail (Swing)", type: "toggle", effect: EFFECT.SESSION, desc: "Trail SL to prev-candle low (CE) / high (PE) on each candle close once trade is in profit (composes with SAR + tier trail, tighten-only)", default: "false" },
      { key: "SWING_PREV_CANDLE_TRAIL_ACTIVATE_PTS", label: "Prev-Candle Trail Activate (pts)", type: "number", min: 5, max: 50, step: 1, effect: EFFECT.SESSION, desc: "Min profit before prev-candle trail starts adjusting SL", default: "15" },
      { key: "SWING_PREV_CANDLE_TRAIL_BUFFER_PTS", label: "Prev-Candle Trail Buffer (pts)", type: "number", min: 0, max: 10, step: 1, effect: EFFECT.SESSION, desc: "Pad below prev-candle low (CE) / above prev-candle high (PE) — avoids wick hunts", default: "2" },
      { key: "SWING_PREV_CANDLE_TRAIL_MAX_GAP", label: "Prev-Candle Trail Max Gap (pts)", type: "number", min: 10, max: 80, step: 5, effect: EFFECT.SESSION, desc: "Skip wide candles — if (close − low) > this, prev-candle trail does NOT fire (prevents loosening on volatile bars)", default: "35" },
  ...
  },
  {
    ...
    fields: [
      ...
      { key: "ORB_TARGET_PCT", label: "Premium Target (%)", type: "number", min: 0.1, max: 1, step: 0.05, effect: EFFECT.INSTANT, desc: "Premium target as fraction of entry premium (0.4 = +40%)", default: "0.4" },
      { key: "ORB_STOP_PCT", label: "Premium SL (%)", type: "number", min: 0.1, max: 0.6, step: 0.05, effect: EFFECT.INSTANT, desc: "Premium SL as fraction of entry premium (0.25 = −25%)", default: "0.25" },
      { key: "ORB_PREMIUM_LOCKIN_PCT",       label: "Premium Lockin Trigger (%)", type: "number", min: 0,    max: 0.5, step: 0.05, effect: EFFECT.INSTANT, desc: "Once option LTP gains this fraction (0.25 = +25%), ratchet premium SL up to entry × (1 + lockin floor). 0 disables.", default: "0.25" },
      { key: "ORB_PREMIUM_LOCKIN_FLOOR_PCT", label: "Premium Lockin Floor (%)",   type: "number", min: 0,    max: 0.2, step: 0.01, effect: EFFECT.INSTANT, desc: "Floor the premium SL ratchets to once lockin triggers — locks in this much profit (0.05 = +5%)", default: "0.05" },
      { key: "ORB_WICK_FILTER_ENABLED", label: "Wick-Rejection Filter", type: "toggle", effect: EFFECT.INSTANT, desc: "Reject breakout candles with large opposing wick (close near mid-candle = weak break)", default: "true" },
      { key: "ORB_MAX_WICK_RATIO", label: "Max Wick Ratio", type: "number", min: 0.2, max: 1.5, step: 0.1, effect: EFFECT.INSTANT, desc: "Upper wick (CE) or lower wick (PE) must be <= this fraction of body. 0.6 = wick can be 60% of body.", default: "0.6" },
      { key: "ORB_VWAP_FILTER_ENABLED", label: "VWAP Alignment Filter", type: "toggle", effect: EFFECT.INSTANT, desc: "CE only if breakout close > session VWAP; PE only if close < VWAP. Falls back to TWAP if candles lack volume.", default: "true" },
      { key: "ORB_VOL_FILTER_ENABLED", label: "Volume Confirmation Filter", type: "toggle", effect: EFFECT.INSTANT, desc: "Require breakout candle volume >= multiplier × prior N-candle avg. Auto-skipped when candles carry no volume (indices have none).", default: "true" },
      ...
      { key: "ORB_PREMIUM_MIN", label: "Min Option Premium (₹)", type: "number", min: 20, max: 500, step: 5, effect: EFFECT.INSTANT, desc: "Skip entry if ATM LTP below this", default: "80" },
      { key: "ORB_PREMIUM_MAX", label: "Max Option Premium (₹)", type: "number", min: 100, max: 1000, step: 10, effect: EFFECT.INSTANT, desc: "Skip entry if ATM LTP above this", default: "250" },
...
const SESSION_RESTART_KEYS = new Set([
  ...
  "SWING_PREV_CANDLE_TRAIL_ENABLED", "SWING_PREV_CANDLE_TRAIL_ACTIVATE_PTS",
  "SWING_PREV_CANDLE_TRAIL_BUFFER_PTS", "SWING_PREV_CANDLE_TRAIL_MAX_GAP",
