...
const SETTINGS_SCHEMA = [
  ...
  },
  {
    ...
    fields: [
      ...
      { key: "ORB_SL_ATR_MULT", label: "Initial Stop — × ATR(5m) ★", type: "number", min: 0.5, max: 3, step: 0.1, effect: EFFECT.INSTANT, desc: "Initial hard stop = the WIDER of the entry candle\u2019s own extreme and this × ATR(5m). The breakout candle is by construction a big momentum bar, so its opposite extreme sits exactly where the normal retracement lives — that stop averaged 23pt and was hit on 4 of 6 trades, including the day that then ran 213pt our way. Rupee risk stays capped by Max Trade Loss and the premium stop, so widening this does NOT widen worst-case loss.", default: "1.5" },
