...
const SETTINGS_SCHEMA = [
  {
    ...
    fields: [
      ...
      { key: "SWING_EMA_FAST", label: "EMA Fast Period", type: "number", min: 5, max: 50, step: 1, effect: EFFECT.INSTANT, desc: "Fast EMA (on close). CE needs EMA-fast ABOVE EMA-slow; PE below. The EMA-fast vs EMA-slow cross is the sole directional gate (EMA9/triple-stack removed 2026-06-19). Classic = 20.", default: "20" },
  ...
  },
  {
    ...
    fields: [
      ...
      { key: "ORB_TRAIL_ENABLED", label: "Continuous Profit Trail", type: "toggle", effect: EFFECT.INSTANT, desc: "Once the option is in profit, continuously ratchet the premium SL up behind the running peak so a winner can't fully round-trip to a loss. Supersedes the one-shot lockin when on.", default: "false" },
      { key: "ORB_TRAIL_ARM_PCT", label: "Trail Arm (%)", type: "number", min: 0.02, max: 0.3, step: 0.01, effect: EFFECT.INSTANT, desc: "Trail only starts once option LTP gains this fraction (0.08 = +8%). Below this it stays on the fixed −Premium SL.", default: "0.08" },
      { key: "ORB_TRAIL_LOCK_PCT", label: "Trail Lock (% of peak profit)", type: "number", min: 0.1, max: 0.9, step: 0.05, effect: EFFECT.INSTANT, desc: "Once armed, the SL is parked to keep this fraction of the running peak profit (0.5 = give back at most half the peak gain). Higher = tighter trail, captures more but exits on smaller pullbacks.", default: "0.5" },
...
const IMMEDIATE_KEYS = new Set([
  ...
  "SWING_EMA_FAST", "SWING_EMA_SLOW", "SWING_EMA_FASTEST",
