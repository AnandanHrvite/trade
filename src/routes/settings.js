...
const SETTINGS_SCHEMA = [
  ...
  },
  {
    ...
    fields: [
      ...
      { key: "ORB_OR_ATR_MAX", label: "Day Filter — Max OR ÷ ATR(15m)", type: "number", min: 1, max: 5, step: 0.1, effect: EFFECT.INSTANT, desc: "Skip the day when the opening range is wider than this × ATR(15m) — the move already happened during the open. Day-sanity guard, not an alpha filter. There is deliberately NO minimum: the two best trades in the Mar–Apr 2026 study came from the two NARROWEST opening ranges, so a floor points the wrong way.", default: "2.5" },
      { key: "ORB_GAP_OR_MULT", label: "Day Filter — Max Gap ÷ OR", type: "number", min: 1, max: 6, step: 0.5, effect: EFFECT.INSTANT, desc: "Skip the day when |today open − prior close| exceeds this × the opening range — an overnight shock far larger than the day\u2019s own structure is news, not a breakout. Fails OPEN when the prior close is unavailable.", default: "3.0" },
      { key: "ORB_BODY_ATR_MULT", label: "Breakout — Min Body ÷ ATR(5m)", type: "number", min: 0.2, max: 1.5, step: 0.1, effect: EFFECT.INSTANT, desc: "The breakout candle\u2019s body must be at least this × ATR(5m) — a decisive bar, not a drift through the level. THE load-bearing entry filter: removing it in the Mar–Apr 2026 study took the worst trade from 0 to −80pt and profit factor from infinite to 3.3. Raise to trade less and better; do not lower without re-measuring.", default: "0.6" },
      { key: "ORB_RETEST_MAX_WAIT", label: "Retest / Resume Window (candles)", type: "number", min: 0, max: 12, step: 1, effect: EFFECT.INSTANT, desc: "If the confirmation candle does not extend the move, stay armed this many 5-min candles for either a trend-resume or a retest-and-hold of the OR edge. A close back through the range cancels the day. 0 disables the fallback (confirmed-breakout entries only).", default: "6" },

      // ── Exits (all owned by src/strategies/orbExits.js — one engine for
      //    paper, live, backtest and scripts/orbValidate.js) ──────────────
      { key: "ORB_SL_ATR_MULT", label: "Initial Stop — \u00d7 ATR(5m)", type: "number", min: 0.5, max: 3, step: 0.1, effect: EFFECT.INSTANT, desc: "Initial hard stop = the WIDER of the entry candle\u2019s own extreme and this \u00d7 ATR(5m), then CLAMPED to whatever \u2018Max Trade Loss\u2019 allows. \u26a0\ufe0f AT THE DEFAULT \u20b91500 / 65-lot THIS KNOB IS INERT: the rupee cap trips after only ~38 spot pts while this stop sits at 50\u201383pt, so every value from 1.0 to 2.5 behaves identically (verified on 39 sessions). To change REAL risk per trade, change \u2018Max Trade Loss\u2019 \u2014 not this. This only starts to matter once the rupee budget is raised above ~\u20b92300.", default: "1.5" },
      ...
      { key: "ORB_ITM_STEPS", label: "Slightly-ITM Strike Steps", type: "number", min: 0, max: 3, step: 1, effect: EFFECT.SESSION, desc: "Trade this many strikes (×50) IN-the-money (CE lower / PE higher) for ~delta 0.6 — tracks the trend move better and decays slower in % than ATM. Costs more per lot, so size down. 0 = ATM. ORB only. NOTE: the ~0.6 delta this implies is also what converts \u2018Max Trade Loss\u2019 into the stop distance, so changing this changes your effective stop width too.", default: "1" },
      ...
      { key: "ORB_MAX_DAILY_LOSS", label: "Max Daily Loss (₹)", type: "number", min: 500, max: 20000, step: 500, effect: EFFECT.SESSION, desc: "ORB daily loss kill-switch. ⚠️ HAS NO EFFECT while 'Max Trades/Day' is 1 (the default): the trade budget is already spent before this can be tested, so the day ends either way. It only becomes live if you raise Max Trades/Day above 1. Per-trade risk is capped by 'Max Loss per Trade (₹)', not by this.", default: "3000" },
      ...
      { key: "ORB_BT_SLIPPAGE_PTS", label: "Backtest Spread/Slippage Haircut (pts each way)", type: "number", min: 0, max: 10, step: 0.5, effect: EFFECT.INSTANT, desc: "Premium points shaved off EACH side (buy higher / sell lower) in /orb-backtest to model the option bid-ask spread + slippage the δ+θ sim cannot see. Backtest only — paper/live fill at the real quote. Default 1.5.", default: "1.5" },
      { key: "ORB_BT_SEED_PREMIUM", label: "Backtest Seed Premium (₹)", type: "number", min: 50, max: 800, step: 10, effect: EFFECT.INSTANT, desc: "Assumed slightly-ITM option entry premium for the /orb-backtest δ+θ P&L sim (there is no historical option chain). Also what the premium-band gate is tested against in the backtest, so a value outside [Min, Max] above skips every trade. Default 240.", default: "240" },

      // ── Debug ────────────────────────────────────────────────────────
      { key: "ORB_DEBUG_TRACE", label: "Debug — Per-Candle Gate Trace", type: "toggle", effect: EFFECT.INSTANT, desc: "Print the full entry funnel to the logs on every 5-min candle close — time window, OR, day sanity, breakout, body, VWAP, confirmation, retest — each PASS/FAIL/SKIP with its numbers, and the final decision. Turn ON for a session when you need to know why entries are not firing, then turn it back OFF (it is verbose).", default: "false" },
    ],
  },
  {
  ...
  },
  {
    ...
    fields: [
      ...
      { key: "GAPS_EXIT_TF", label: "Exit Candle Timeframe (min)", type: "select", options: ["1", "3", "5", "10", "15", "30", "60"], effect: EFFECT.SESSION, desc: "The intraday timeframe the trailing EMA is built on, and whose CLOSE triggers the trail exit. Also the candle size drawn on the intraday chart. Default 5.", default: "5" },

      // ── Exits ──
      { key: "GAPS_TRAIL_ENABLED", label: "EMA Trailing Stop", type: "toggle", effect: EFFECT.INSTANT, desc: "Trail the position with the intraday EMA (period set by Trail EMA Length) and exit when a candle CLOSES back THROUGH it — a PE exits on a close ABOVE the EMA, a CE on a close BELOW. This is a trailing stop, not a target: the level follows price every candle. Turn OFF to run gap-fill-stop-and-EOD only. Default on.", default: "true" },
  ...
  },
  {
    ...
    fields: [
      ...
      { key: "EMA9VWAP_OI_ENABLED", label: "OI Filter (EMA9+VWAP)", type: "toggle", effect: EFFECT.INSTANT, desc: "Apply the OI buildup filter to EMA9+VWAP entries. Requires the MASTER switch ON. Default OFF.", default: "false" },
  ...
  },
  {
    ...
    fields: [
      ...
      { key: "OPTION_CHAIN_RECORD_INTERVAL_SEC", label: "Option-Chain Record Interval (sec)", type: "number", min: 2, max: 60, step: 1, effect: EFFECT.INSTANT, desc: "How often the day-wide recorder polls the chain. 5s ≈ one getQuotes call per 5s. Lower = denser replay data + more API load.", default: "5" },
      { key: "OPTION_CHAIN_RECORD_STRIKES", label: "Option-Chain Strikes (ATM±N)", type: "number", min: 1, max: 15, step: 1, effect: EFFECT.INSTANT, desc: "How many strikes each side of ATM to record (per CE+PE). 5 = 11 strikes × 2 sides = 22 option symbols per poll.", default: "5" },
      { key: "SPOT_FEED_ALWAYS_ON", label: "Always-On Spot Feed (day recording)", type: "toggle", effect: EFFECT.INSTANT, desc: "Keep the shared NIFTY spot feed connected 09:15–15:30 IST even when NO strategy is running, so every trading day is recorded in full for Replay. Without this, a day with no strategy started records nothing, and stopping the last strategy ends that day's recording early. Pure observer — opens no second socket, registers no tick callback, places no order, and never stops a feed a strategy is using.", default: "true" },
      ...
      { key: "LIVE_EXIT_WAIT_MS", label: "Live Exit Wait Ceiling (ms)", type: "number", min: 0, max: 120000, step: 1000, effect: EFFECT.INSTANT, desc: "How long a live square-off may be waited on before the app gives up waiting and alerts you. Neither broker SDK sets an HTTP timeout, so without this a dead broker socket could hang the Stop button and stall a deploy. It does NOT cancel the order — a sent market order cannot be recalled — it only stops waiting, and the alert says to verify the broker dashboard. A healthy order takes well under a second, so 20000 never fires in normal use. 0 = wait forever (not recommended).", default: "20000" },
