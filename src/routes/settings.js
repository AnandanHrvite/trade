...
const SETTINGS_SCHEMA = [
  ...
  },
  {
    ...
    fields: [
      ...
      { key: "ORB_BREAKEVEN_PTS", label: "Exit — Breakeven After (pts)", type: "number", min: 0, max: 60, step: 5, effect: EFFECT.INSTANT, desc: "Once the trade is this many NIFTY points in profit, lift the hard SL to the entry price. 0 = never move to breakeven.", default: "20" },
      ...
      { key: "ORB_MAX_RANGE_PTS", label: "Max OR Width (pts)", type: "number", min: 50, max: 300, step: 10, effect: EFFECT.INSTANT, desc: "Skip when opening range is too wide — exhausted move (open already ran)", default: "100" },
      ...
      { key: "ORB_BREAKOUT_BUFFER_MIN", label: "Breakout Buffer — Min (pts)", type: "number", min: 0, max: 40, step: 1, effect: EFFECT.INSTANT, desc: "Entry needs the close to CLEAR the OR edge (not just touch it) by max(this, pct×range). Filters bare-touch false breakouts that reverse straight back.", default: "8" },
      { key: "ORB_BREAKOUT_BUFFER_PCT", label: "Breakout Buffer — % of Range", type: "number", min: 0, max: 0.5, step: 0.05, effect: EFFECT.INSTANT, desc: "The other half of the breakout buffer: fraction of OR width. 0.15 = clear the edge by 15% of the range. Buffer used = max(min-pts, this × range).", default: "0.15" },
      { key: "ORB_RETEST_ENABLED", label: "Retest-Entry Gate (backtest only)", type: "toggle", effect: EFFECT.INSTANT, desc: "EXPERIMENTAL, backtest only for now. Instead of buying the breakout candle, wait for price to pull back to the broken OR edge and CLOSE back on the breakout side (level held), then enter. Filters poke-and-reverse false breakouts — but any day that never pulls back takes NO trade (you miss runaway-trend days). Not yet wired into paper/live.", default: "false" },
      { key: "ORB_RETEST_TOL_MIN", label: "Retest — Tolerance Min (pts)", type: "number", min: 0, max: 30, step: 1, effect: EFFECT.INSTANT, desc: "How close price must return to the OR edge to count as a retest: within max(this, pct×range) points. Larger = looser (more entries, closer to immediate-entry behaviour).", default: "5" },
      ...
      { key: "ORB_RETEST_MAX_WAIT", label: "Retest — Max Wait (candles)", type: "number", min: 1, max: 20, step: 1, effect: EFFECT.INSTANT, desc: "Give up and take no trade if the retest hasn't happened within this many 5-min candles after the breakout.", default: "6" },
