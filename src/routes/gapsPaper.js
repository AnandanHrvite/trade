...
// optionFeed, so this loop's REST call fires only when the stream is quiet or
// unavailable. The loop itself stays — it is what renews the optionFeed lease,
// and it is the fallback that keeps behaviour identical if the stream is down.
const OPTION_POLL_MS = 3000;
const OPTION_FEED_OWNER = "gaps-paper";
...
function _applyOptionLtp(symbol, ltp, at, record) {
  ...
  if (record) { try { tickRecorder.recordOptionLtp(symbol, ltp, "gaps-paper"); } catch (_) {} }
}
