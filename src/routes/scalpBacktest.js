...
${modalJS()}
  ...
  if (!jobId) {
    ...
    if (activeJob) {
      return res.send(backtestJobs.buildProgressPage(activeJob.id, '/scalp-backtest', 'Scalp Backtest'));
    }
...
<div class="page">
  ...
  <div class="run-bar">
    ...
    <button class="run-btn" onclick="(function(){var f=document.getElementById('f').value,t=document.getElementById('t').value,r=document.getElementById('r').value;if(!f||!t){showAlert({icon:'\u26a0\ufe0f',title:'Missing Dates',message:'Set both From and To dates'});return;}if(document.getElementById('splitYears').checked){var fy=parseInt(f.split('-')[0]),ty=parseInt(t.split('-')[0]);for(var y=fy;y<=ty;y++){var yf=(y===fy)?f:y+'-01-01',yt=(y===ty)?t:y+'-12-31';window.open('/scalp-backtest?from='+yf+'&to='+yt+'&resolution='+r,'_blank');}}else{window.location='/scalp-backtest?from='+f+'&to='+t+'&resolution='+r;}})()">🔄 Run Again</button>
