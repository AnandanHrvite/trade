...
async function squareOff(exitPrice, reason) {
  ...
  const _exitTrade = {
    ...
    bandWidthAtEntry:  state.position ? (state.position.bandWidthAtEntry  != null ? state.position.bandWidthAtEntry  : null) : null,
    rsiRangeAtEntry:   state.position ? (state.position.rsiRangeAtEntry   != null ? state.position.rsiRangeAtEntry   : null) : null,
    divergenceAtEntry: state.position ? (state.position.divergenceAtEntry || null) : null,
...
function onTick(tick) {
  ...
  if (state.position) {
    ...
    // 0. MIDDLE-BAND TARGET — the objective of a mean-reversion trade. Mirrors
    //    bbRsiPaper (the canonical route): price stretched away from the mean and we
    //    are paid when it returns to it, so the middle band IS the take-profit. Runs
    //    per-tick rather than at candle close so the touch is banked at the line.
    if (state.candles.length >= 15) {
      ...
      if (_tgt.hit) {
        ...
        squareOff(price, `BB middle target (${_tgt.level})`).catch(e => console.error(`🚨 [BB_RSI] squareOff error: ${e.message}`));
        return;
      }
    }

    // 0b. GLOBAL PROFIT LOCK (shared across every strategy — see tradeGuards).
    //     Mirrors bbRsiPaper: AFTER the middle-band target so this mean-reversion
    //     strategy's own take-profit still wins when both fire on the same tick,
    //     and BEFORE every stop below (once armed, the floor sits above entry).
    if (pos.optionEntryLtp && state.optionLtp) {
      ...
      if (_plMsg) {
        console.log(`🔒 [BB_RSI-LIVE] ${_plMsg}`);
        pos.slSource = "Profit lock";
        squareOff(price, _plMsg).catch(e => console.error(`🚨 [BB_RSI] squareOff error: ${e.message}`));
        return;
      }
    }
  ...
  // skipped (a poke that closes back the other way is not a reversal, and this
  // engine is fading an extreme).
...
async function onCandleClose(bar) {
  ...
  // bar, not an intra-bar poke that closed back the other way. Entry at this close.
  ...
  if (!state.position && state._armedSignal && confirmCandle.enabled("BB_RSI")
      && confirmCandle.onCloseEnabled("BB_RSI") && isMarketHours()) {
    const _a = state._armedSignal;
  ...
  if (state.position) {
    ...
    // Two-opposite-candle stop / trail — the only candle-close exit (mirrors paper).
    // Before the trade has run BB_RSI_TRAIL_ARM_PTS in favour this is the initial stop;
    // after, the trail takes over with its own count. Independently toggleable.
    {
      ...
      if (_opp.hit) {
        ...
        squareOff(bar.close, _opp.reason).catch(e => console.error(`🚨 [BB_RSI] squareOff error: ${e.message}`));
        return;
      }
    }
  ...
  if (result.signal === "NONE") {
    ...
    log(`⏭️ [BB_RSI-LIVE] SKIP: ${result.reason} | Close=${lastBar.close} BB=[${result.bbLower||'?'},${result.bbUpper||'?'}] width=${result.bbWidth||'?'} RSI=${result.rsi||'?'}`);
...
async function resolveAndEnter(side, spot, result) {
  ...
  try {
    ...
    // engine: CE triggers below the LOWER band, PE above the UPPER (mirrors paper).
    let _ptsFromBB = null;
    ...
    state.position = {
      ...
      // Entry context — captured for the trade log.
...
router.get("/start", async (req, res) => {
  ...
  scheduleAutoStop((msg) => {
    ...
    stopSession().catch(e => log(`⚠️ [BB_RSI-LIVE] Auto-stop failed: ${e.message}`));
  });
...
// PARITY with bbRsiPaper (canonical): paper's exit is synchronous, so its trade is
// in state.sessionTrades and state.sessionPnl is final before any bookkeeping runs.
// squareOff() here is a broker round-trip and used to be fired un-awaited, so
// execution ran on to saveBbRsiSession() and notifyDayReport() while the sell was
// still in flight — the saved session was missing its final trade and its P&L.
...
<div class="top-bar">
  <div>
    ...
    <div class="top-bar-meta">${bbRsiStrategy.NAME} \u00b7 ${BB_RSI_RES}-min candles \u00b7 ${bbRsiStrategy.direction() === "breakout" ? "Break entry \u00b7 No fixed target" : "Fade entry \u00b7 Target: BB middle"} \u00b7 SL: 2 opposite candles \u00b7 ${state.running ? "Auto-refreshes 2s" : "Not refreshing"}</div>
  </div>
...
<div class="page">
  ...
  <div class="stat-grid">
    ...
    <div class="sc" style="border-top:1.5px solid #8b5cf6;">
      ...
      <div class="sc-val"><span id="ax-trade-count">${state.sessionTrades.length}</span> <span style="font-size:0.75rem;color:var(--muted-1,#8ba1c2);">/ ${_BB_RSI_MAX_TRADES}</span></div>
          ...
          <div style="background:#0d1320;border:1px solid #3a1a1a;border-radius:16px;padding:24px 28px;max-width:720px;width:100%;max-height:90vh;overflow-y:auto;box-shadow:0 24px 80px rgba(0,0,0,0.9);position:relative;">
            <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:18px;">
              <div>
                ...
                <span style="font-size:0.65rem;color:var(--muted-1,#8ba1c2);margin-left:10px;">\uD83D\uDD34 BB_RSI Live \u2014 Full Details</span>
              </div>
