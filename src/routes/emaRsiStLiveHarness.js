...
router.get("/start", async (req, res) => {
  ...
  if (!liveDryRun.isDryRun() && (process.env.EMA_RSI_ST_LIVE_ENABLED || "false").toLowerCase() !== "true") {
...
async function startSession() {
  ...
  try {
    const r = await secretFetch('/ema_rsi_st-live-harness/start');
...
async function stopSession() {
  ...
  try {
    const r = await secretFetch('/ema_rsi_st-live-harness/stop');
