 ...
 *
 * Routes:
 *   GET /pa-backtest  → Run backtest with query params
 * ─────────────────────────────────────────────────────────────────────────────
 */
...
function runPABacktest(candles, capital, vixCandles, expiryDates) {
  ...
  // PA config
  const PA_MAX_TRADES    = parseInt(process.env.PA_MAX_DAILY_TRADES || "30", 10);
  const PA_MAX_LOSS      = parseFloat(process.env.PA_MAX_DAILY_LOSS || "2000");
  ...
  // Build daily OHLC for day boundary tracking
  ...
  for (let i = 30; i < candles.length; i++) {
    ...
    if (position) {
      ...
      //    Once peak PnL >= PA_TRAIL_START, use prev candle high/low as trail level.
      //    But never exit below the profit-lock floor (tiered % of peak).
      //    This lets winners ride trends while protecting from total giveback.
      if (!exitReason && PA_TRAIL_START > 0 && position.peakPnl >= PA_TRAIL_START) {
        // Compute profit-lock floor (always active as safety net)
        ...
        if (position.candleTrailLevel) {
          ...
          if (candleBreached) {
            const candleExitPrice = parseFloat((position.candleTrailLevel + SLIPPAGE_PTS * (position.side === "CE" ? -1 : 1)).toFixed(2));
            const candleExitPnl = _runPnl(candleExitPrice);
            // Only exit via candle trail if PnL stays above profit-lock floor
            if (candleExitPnl >= trailFloor) {
              exitPrice  = candleExitPrice;
              exitReason = `Candle Trail (prev ${position.side === "CE" ? "low" : "high"} ${position.candleTrailLevel} | floor ₹${trailFloor})`;
            }
          }
        }

        // Fallback: profit-lock floor breach (safety net)
        if (!exitReason) {
    ...
    if (_dailyTradeCount >= PA_MAX_TRADES) continue;
    ...
    if (result.signal === "NONE") {
      ...
      if (!position && _dailyTradeCount === 0 && candleMin >= _btStartMin && candleMin < _btEndMin) {
        if (!_loggedReason || _loggedReason !== candleDate) {
          console.log(`  [${candleDate} ${toIST(candle.time).split(' ')[1] || ''}] Skip: ${result.reason} | RSI=${result.rsi} Pattern=${result.pattern || 'none'} SR=${result.srLevel || '-'}`);
    ...
    pendingSignal = {
      ...
      maxSlPts: parseFloat(process.env.PA_MAX_SL_PTS || "25"),
  ...
  console.log(`\n📊 PRICE ACTION BACKTEST RESULT: ${trades.length} trades | WR ${winRate}% | PnL ${inr(totalPnl)}`);
...
<title>${title} — Price Action Backtest</title>
...
router.get("/", async (req, res) => {
  ...
  if (liveActive) {
    ...
    return res.status(503).send(`<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"/>
      <meta name="viewport" content="width=device-width,initial-scale=1"/>
      <title>Price Action Backtest blocked — Live trade active</title>
...
<div class="main-content">
      <div style="display:flex;align-items:center;justify-content:center;flex:1;padding:40px;">
        <div style="background:#0d1320;border:1px solid #7f1d1d;border-radius:14px;padding:40px 48px;max-width:480px;text-align:center;">
          ...
          <h2 style="color:#ef4444;margin-bottom:12px;font-size:1.1rem;">Price Action Backtest blocked</h2>
...
<head>
  ...
  <title>Price Action Backtest — ${paStrategy.NAME}</title>
  <style>
...
<body>
<div class="app-shell">
${buildSidebar('paBacktest', liveActive)}
<div class="main-content">
...
function renderAnalytics(){
  ...
  trades.forEach(function(t){
    ...
    else if(r.indexOf('Swing')>=0) r='Swing Trail SL';
    else if(r.indexOf('SL hit')>=0) r='SL hit';
    else if(r.indexOf('Pattern reversal')>=0) r='Pattern reversal';
...
document.getElementById('btModal').addEventListener('click',function(e){
  if(e.target===this) this.style.display='none';
    ...
    console.error("PA backtest error:", err);
