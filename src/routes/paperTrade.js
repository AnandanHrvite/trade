...
async function onCandleClose(candle) {
  ...
  if (ptState.position && stopLoss !== null && stopLoss !== undefined) {
    ...
    if (tighten) {
      ...
      log(`🔄 [PAPER] SAR tightened: ₹${oldSL} → ₹${stopLoss} (${pos.side==="CE"?"↑CE":"↓PE"})`);
    } else {
...
<div class="page">
  ...
  <div style="display:flex;align-items:center;gap:10px;margin-bottom:16px;flex-wrap:wrap;padding:12px 16px;background:#0d1320;border:1px solid #1a2236;border-radius:10px;">
    ...
    <div>
      ...
      <div id="ajax-current-capital" style="font-size:1.1rem;font-weight:700;font-family:monospace;color:${data.capital >= getCapitalFromEnv() ? '#10b981' : '#ef4444'};">₹${data.capital.toLocaleString("en-IN", {minimumFractionDigits:2,maximumFractionDigits:2})}</div>
    </div>
    <div style="width:1px;height:36px;background:#1a2236;"></div>
    <div>
      ...
      <div id="ajax-alltime-pnl" style="font-size:1.1rem;font-weight:700;font-family:monospace;color:${data.totalPnl >= 0 ? '#10b981' : '#ef4444'};">${data.totalPnl >= 0 ? '+' : ''}₹${data.totalPnl.toLocaleString("en-IN", {minimumFractionDigits:2,maximumFractionDigits:2})}</div>
    </div>
    <div style="width:1px;height:36px;background:#1a2236;"></div>
