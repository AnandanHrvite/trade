...
function rehydrateSessionFromJsonl() {
  try {
    ...
    const seen = new Set();
    for (const s of (loadPAData().sessions || [])) {
    ...
    console.log(`♻️ [PA-PAPER] Restart recovery — restored ${missing.length} trade(s) from today's JSONL (sessionPnl ₹${state.sessionPnl})`);
  } catch (err) {
