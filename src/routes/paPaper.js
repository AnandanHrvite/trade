...
function onTick(tick) {
  ...
  if (!state.currentBar || state.barStartTime !== bucketMs) {
    ...
    if (state.currentBar) {
      ...
      closeAndEvaluate(closedBar).catch(e => console.error(`🚨 [PA-PAPER] closeAndEvaluate error: ${e.message}`));
    }
...
async function closeAndEvaluate(closedBar) {
  ...
  if (!state._simMode) {
    try {
      ...
      const histCandles = await fetchCandles(NIFTY_INDEX_SYMBOL, String(PA_RES), istDate, istDate);
      ...
      if (match) {
        ...
        if (match.high !== closedBar.high || match.low !== closedBar.low || match.open !== closedBar.open || match.close !== closedBar.close) {
          ...
          log(`📊 [PA-PAPER] Auth candle ${hhmm}: tick O=${closedBar.open} H=${closedBar.high} L=${closedBar.low} C=${closedBar.close} → hist O=${match.open} H=${match.high} L=${match.low} C=${match.close}`);
        }
      } else {
        const hhmm = new Date(closedBar.time * 1000).toLocaleTimeString("en-IN", { timeZone: "Asia/Kolkata", hour12: false });
        log(`⚠️ [PA-PAPER] Auth candle not yet in history for ${hhmm} — using tick-built bar`);
      }
    } catch (e) {
      log(`⚠️ [PA-PAPER] Auth candle fetch failed (${e.message}) — using tick-built bar`);
    }
  }
...
function getPACapitalFromEnv() {
  const v = parseFloat(process.env.PA_PAPER_CAPITAL);
...
router.get("/history", (req, res) => {
  ...
  const startCap = getPACapitalFromEnv();
  ...
  const sessionCards = data.sessions.length === 0
    ? `<div style="text-align:center;padding:60px 24px;background:#07111f;border:0.5px solid #0e1e36;border-radius:12px;">
        ...
        <div style="font-size:0.82rem;color:#4a6080;">Start price action paper trading to record your first session.</div>
       </div>`
...
<head>
  ...
  <link rel="icon" href="data:image/svg+xml,<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 100 100'><text y='.9em' font-size='90'>⚡</text></svg>">
  <title>Price Action Paper — History</title>
  <link href="https://fonts.googleapis.com/css2?family=Inter:wght@400;500;600;700&family=IBM+Plex+Mono:wght@400;600&display=swap" rel="stylesheet"/>
...
<body>
<div class="app-shell">
${buildSidebar('paHistory', liveActive)}
<div class="main-content">
  <div class="top-bar">
    <div>
      <div class="top-bar-title">⚡ Price Action Paper Trade History</div>
  ...
  <div style="background:#0d1320;border:1px solid #1d3b6e;border-radius:16px;padding:24px 28px;max-width:720px;width:100%;max-height:90vh;overflow-y:auto;box-shadow:0 24px 80px rgba(0,0,0,0.9);position:relative;">
    <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:18px;">
      <div>
        ...
        <span style="font-size:0.65rem;color:#4a6080;margin-left:10px;">Price Action Paper — Trade Details</span>
      </div>
...
async function confirmReset() {
  ...
  if (!ok) return;
  try {
    var r = await secretFetch('/pa-paper/reset');
    if (!r) return;
    var d;
...
async function deleteSession(idx, label) {
  ...
  if (!ok) return;
  try {
    var r = await secretFetch('/pa-paper/session/' + idx, { method: 'DELETE' });
    if (!r) return;
    var d;
    try { d = await r.json(); } catch(_) { d = { success: false, error: 'Server error' }; }
    if (d.success) { window.location.reload(); }
...
function copyTradeLog(btn){
  ...
  ALL_TRADES_JSON.forEach(function(t){
    var ed=t.entryTime?(t.entryTime.split(', ')[0]||''):'';
    var et=t.entryTime?(t.entryTime.split(', ')[1]||''):'';
    var xt=t.exitTime?(t.exitTime.split(', ')[1]||''):'';
    lines.push((t.side||'')+'\\t'+ed+'\\t'+(t.spotAtEntry||t.entryPrice||'')+'\\t'+et+'\\t'+(t.spotAtExit||t.exitPrice||'')+'\\t'+xt+'\\t'+(t.stopLoss||'')+'\\t'+(t.pnl!=null?t.pnl.toFixed(2):'')+'\\t'+(t.entryReason||'')+'\\t'+(t.exitReason||''));
  });
  doCopy(lines.join('\\n'),btn,'Trade Log');
}
