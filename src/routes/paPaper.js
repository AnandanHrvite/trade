        ...
        <div class="session-card" data-session-idx="${actualIdx}" data-trade-count="${trades.length}" data-wins="${sessionWins}" data-losses="${sessionLosses}" data-pnl="${s.pnl}">
          ...
          <div style="overflow-x:auto;">
            <div class="tbl-wrap"><table class="tbl enh-table-sort-filter">
              <thead><tr>
                <th>Side</th><th>Pattern</th><th>Date</th><th>Entry</th><th>Entry Time</th><th>Exit</th><th>Exit Time</th><th>SL</th><th>PnL</th><th>Entry Reason</th><th>Exit Reason</th><th data-no-sort="1" style="text-align:center;">Action</th>
              </tr></thead>
...
async function loadDailyFiles(){
  try {
    var res = await fetch('/pa-paper/download/daily-files', { cache: 'no-store' });
    var d = await res.json();
