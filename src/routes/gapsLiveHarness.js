/**
 * GAPS LIVE (HARNESS) — /gaps-live
 * ─────────────────────────────────────────────────────────────────────────────
 * Runs GAPS LIVE by wrapping GAPS PAPER with the live harness:
 *   1. Install harness (registers notify entry/exit hooks → real Fyers orders)
 *   2. Trigger /gaps-paper/start programmatically (paper code runs unchanged)
 ...
 * whatever gapsPaper says it is (single source of truth).
 *
 * TRIPLE-GATED to dry-run — real orders require ALL of:
 *   GAPS_LIVE_ENABLED=true        (per-strategy live enable, default false)
 *   LIVE_HARNESS_DRY_RUN=false    (global kill-switch, default true)
 *   GAPS_LIVE_DRY_RUN!=true       (per-strategy dry-run override)
...
function _invokePaperRoute(method, urlPath) {
  return new Promise((resolve, reject) => {
    ...
    const stack = gapsPaperRoute.stack || [];
    let i = 0;
    function next(err) {
...
router.get("/status/data", async (req, res) => {
  const cfg = liveHarness.getConfig("GAPS-LIVE");
  let paperData = {};
  try {
  ...
  res.json({
    ...paperData,
    installed:    liveHarness.isInstalled("GAPS-LIVE"),
    config:       cfg,
    recentEvents: liveHarness.getRecentEvents(50, "GAPS-LIVE"),
  });
});

router.get("/start", async (req, res) => {
  if (liveHarness.isInstalled("GAPS-LIVE")) {
    return res.status(409).json({ success: false, error: "GAPS-LIVE harness is already running. Stop it first." });
  }

  if (String(process.env.GAPS_MODE_ENABLED || "true").toLowerCase() !== "true") {
    return res.status(403).json({ success: false, error: "GAPS mode is disabled. Enable GAPS Mode in Settings first." });
  }

  // Default DRY-RUN unless the user explicitly set LIVE_HARNESS_DRY_RUN=false
  // (and GAPS_LIVE_DRY_RUN is not true).
  const dryRun = liveDryRun.isDryRun("GAPS");
  ...
  try {
    installed = liveHarness.installHarness({
      mode:       "GAPS-LIVE",
      modeTag:    "GAPS-PAPER",       // gapsPaper's mode field in notify payloads
      broker:     "fyers",
      dryRun,
      isFutures:  process.env.INSTRUMENT === "NIFTY_FUTURES",
      liveLogKey: "gaps-live",
    });
  } catch (err) {
  ...
  try {
    ...
    if (startResp.status >= 400 && startResp.status !== 302) {
      liveHarness.uninstallHarness("GAPS-LIVE");
      return res.status(startResp.status).json({
        success: false,
        error:   `gapsPaper /start failed: ${JSON.stringify(startResp.body).slice(0, 300)}`,
      });
    }
    return res.json({
      success: true,
      ...
      message: installed.dryRun
        ? "GAPS-LIVE harness started in DRY-RUN. Decisions match paper, no real orders placed. Watch /gaps-live."
        : "GAPS-LIVE harness started — real Fyers orders WILL be placed.",
...
router.get("/stop", async (req, res) => {
  ...
  try {
    ...
    liveHarness.uninstallHarness("GAPS-LIVE");
    return res.json({ success: true, message: "GAPS-LIVE harness stopped + paper session ended.", paperStopResp: stopResp });
  } catch (err) {
    try { liveHarness.uninstallHarness("GAPS-LIVE"); } catch (_) {}
    return res.status(500).json({ success: false, error: err.message });
  }
});
...
router.get("/", (req, res) => {
  const cfg = liveHarness.getConfig("GAPS-LIVE");
  const installed = liveHarness.isInstalled("GAPS-LIVE");
  const dryRunCurrent = liveDryRun.isDryRun("GAPS");
  const scfg = gapsStrategy.getConfig();
  const html = `<!DOCTYPE html>
<html><head>
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width,initial-scale=1">
<title>GAPS LIVE (Harness) — Real orders via Paper engine</title>
${faviconLink()}
<style>
${sidebarCSS()}
${modalCSS()}
...
<div class="main">
  ...
  <div class="card">
    <div class="section-title">Today — gap vs yesterday</div>
    <div class="row" id="gaps-live-context"><div class="val">Loading…</div></div>
  </div>
...
async function startSession() {
  ...
  try {
    const r = await secretFetch('/gaps-live/start');
...
async function stopSession() {
  ...
  try {
    const r = await secretFetch('/gaps-live/stop');
