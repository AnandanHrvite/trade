...
function _floatEnv(key, def) {
  const v = parseFloat(process.env[key]);
  return Number.isFinite(v) ? v : def;
}

function _pollMs()          { return _intEnv("EARLYBIRD_POLL_MS", 2000, 500, 30000); }
function _historyLagMs()    { return _intEnv("EARLYBIRD_HISTORY_LAG_MS", 5000, 0, 120000); }
/** Calendar days of daily history to reach back for the previous close. */
function _warmupDays()      { return _intEnv("EARLYBIRD_WARMUP_DAYS", 10, 3, 120); }
function _maxDailyTrades()  { return _intEnv("EARLYBIRD_MAX_DAILY_TRADES", 5, 1, 100); }
function _maxDailyLoss()    { return _floatEnv("EARLYBIRD_MAX_DAILY_LOSS", 5000); }
function _maxDailyLosses()  { return _intEnv("EARLYBIRD_MAX_DAILY_LOSSES", 3, 0, 100); }
function _dailyProfitLock() { return _floatEnv("EARLYBIRD_DAILY_PROFIT_LOCK", 0); }
function _maxWeeklyLoss()   { return _floatEnv("EARLYBIRD_MAX_WEEKLY_LOSS", 0); }
/** Symbols per getQuotes call. Fyers accepts a comma list; keep it modest. */
function _quoteChunk()      { return _intEnv("EARLYBIRD_QUOTE_CHUNK", 20, 1, 50); }
/** A quote older than this stops being used to test an exit. */
function _quoteStaleSec()   { return _intEnv("EARLYBIRD_QUOTE_STALE_SEC", 30, 5, 600); }
/** Concurrent history fetches during the 09:30 scan. */
function _scanConcurrency() { return _intEnv("EARLYBIRD_SCAN_CONCURRENCY", 4, 1, 12); }
...
// chunking here. The scanner's own rate limiter is not reachable from outside
// that module, so concurrency is capped instead — EARLYBIRD_SCAN_CONCURRENCY,
// default 4, the same order as SWING_SCANNER_CONCURRENCY.
// ─────────────────────────────────────────────────────────────────────────────
...
function _equityCharges(qty, entryPrice, exitPrice) {
  ...
  const brokerage = Math.min(_floatEnv("EARLYBIRD_BROKERAGE_PER_ORDER", 20), 0.0003 * buyTurnover)
                  + Math.min(_floatEnv("EARLYBIRD_BROKERAGE_PER_ORDER", 20), 0.0003 * sellTurnover);
  const stt   = _floatEnv("EARLYBIRD_STT_PCT",   0.025)   / 100 * sellTurnover;   // intraday: sell side only
  const txn   = _floatEnv("EARLYBIRD_TXN_PCT",   0.00297) / 100 * turnover;
  const sebi  = _floatEnv("EARLYBIRD_SEBI_PCT",  0.0001)  / 100 * turnover;
  const stamp = _floatEnv("EARLYBIRD_STAMP_PCT", 0.003)   / 100 * buyTurnover;    // intraday: buy side only
