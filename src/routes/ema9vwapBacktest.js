...
const { runEma9VwapBacktest } = require("../services/ema9vwapBacktestEngine");
const ACTIVE = "EMA9_VWAP";
...
<div class="main-content">
      <div style="display:flex;align-items:center;justify-content:center;flex:1;padding:40px;">
        <div style="background:#0d1320;border:1px solid #7f1d1d;border-radius:14px;padding:40px 48px;max-width:480px;text-align:center;">
          ...
          <a href="/ema9vwap-live" style="background:#dc2626;color:#fff;padding:9px 22px;border-radius:8px;text-decoration:none;font-weight:600;font-size:0.85rem;">→ Go to Live Trade</a>
        </div>
      </div>
      </div></div></body></html>`);
  }
  ...
  if (!jobId) {
    ...
    if (activeJob) {
      return res.send(backtestJobs.buildQueuePage('/ema9vwap-backtest', 'EMA9+VWAP Backtest'));
    }

    const { id } = backtestJobs.createJob('ema9vwap');
    ...
    return res.send(backtestJobs.buildProgressPage(id, '/ema9vwap-backtest', 'EMA9+VWAP Backtest'));
  }

  // ── Render results from completed job ──────────────────────────────────────
  const job = backtestJobs.getJob(jobId);
  if (!job) return res.redirect('/ema9vwap-backtest');
  if (job.status === 'running') return res.send(backtestJobs.buildProgressPage(jobId, '/ema9vwap-backtest', 'EMA9+VWAP Backtest'));
  if (job.status === 'error') {
...
<head>
  ...
  <title>EMA9+VWAP — Backtest</title>
  <style>
...
<body>
<div class="app-shell">
${buildSidebar('ema9vwapBacktest', liveActive)}
<div class="main-content">
...
<div class="page">
  ...
  <div id="anaWrap" style="display:none;margin-bottom:16px;" class="ana-panel">
    ...
    <div class="ana-row3">
      ...
      <div class="ana-mini">
        <h3>📥 Entry Reason Breakdown</h3>
        <div style="overflow-x:auto;"><table class="ana-tbl"><thead><tr><th>Reason</th><th>Count</th><th>Wins</th><th>Losses</th><th>WR%</th><th>P&L</th><th>Avg</th></tr></thead><tbody id="anaEntryBody"></tbody></table></div>
      </div>
      <div class="ana-mini">
    ...
    <div style="background:#0d1320;border:1px solid #1d3b6e;border-radius:16px;padding:24px 28px;max-width:680px;width:100%;max-height:90vh;overflow-y:auto;box-shadow:0 24px 80px rgba(0,0,0,0.9);position:relative;">
      <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:18px;">
        <div>
          ...
          <span style="font-size:0.65rem;color:var(--muted-1,#8ba1c2);margin-left:10px;">🔍 Backtest — Full Details</span>
        </div>
...
function renderAnalytics(){
  ...
  entryReasons.forEach(function(r){
    ...
    entryHtml2+='<tr><td style="color:#c8d8f0;max-width:200px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;" title="'+r+'">'+r+'</td><td>'+d.cnt+'</td>'
  ...
  document.getElementById('anaEntryBody').innerHTML=entryHtml2;
...
function buildBacktestPageWithToast(from, to, resolution, errMsg, liveActive) {
  ...
  <style>
    *{box-sizing:border-box;margin:0;padding:0;}
    ...
    label{font-size:0.75rem;color:var(--muted-1,#8ba1c2);display:block;margin-bottom:4px;}
    ...
    .run-btn{background:#2563eb;color:#fff;border:none;border-radius:8px;padding:9px 20px;font-weight:600;font-size:0.83rem;cursor:pointer;display:flex;align-items:center;gap:6px;}
    .run-btn:hover{background:#2563eb;}
  ...
  <div id="err-toast">
    <div style="display:flex;align-items:flex-start;gap:10px;">
      ...
      <span onclick="this.closest('#err-toast').remove()" style="margin-left:auto;color:var(--muted-1,#8ba1c2);font-size:1.1rem;cursor:pointer;flex-shrink:0;padding-left:8px">✕</span>
    </div>
  </div>
  <div class="page">
    <div style="margin-bottom:20px;">
      <div style="font-size:1.4rem;font-weight:700;color:#fff;margin-bottom:4px;">EMA9+VWAP Backtest</div>
      <div style="font-size:0.78rem;color:var(--muted-1,#8ba1c2);">Fix the error above and run again.</div>
    </div>
    <div class="card">
...
function errorPage(title, msg, from, to, resolution) {
  return `<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"/>
  <meta name="viewport" content="width=device-width,initial-scale=1"/><link rel="icon" type="image/png" href="data:image/png;base64,"/><title>${title}</title>
  ...
  h2{color:#ef4444;margin-bottom:12px;font-size:1.1rem;}p{font-size:0.83rem;color:var(--muted-1,#8ba1c2);margin-bottom:24px;line-height:1.6;}
  a{background:#2563eb;color:#fff;padding:9px 20px;border-radius:8px;text-decoration:none;font-weight:600;font-size:0.83rem;}
