...
const { istDayFromAny, istIsoFromAny, reverseSlice, formatISTTimestamp, fmtISTDateTime, getISTMinutes: _getISTMinutesReal, getBucketStart: _getBucketStartRaw, isPreMarketBucket, parseOptionDetails, parseTimeToMinutes } = require("../utils/tradeUtils");
...
const OPTION_FEED_OWNER = "bb_rsi-paper";
...
router.get("/status", (req, res) => {
  ...
  const posHtml = pos ? `
    <div style="background:#0a1f0a;border:1px solid #065f46;border-radius:12px;padding:20px 24px;">
      ...
      <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(130px,1fr));gap:10px;margin-bottom:12px;">
        ...
        <div style="background:#1c1400;border:1px solid #78350f;border-radius:8px;padding:12px 14px;">
          <div style="font-size:0.6rem;color:var(--muted-1,#8ba1c2);text-transform:uppercase;letter-spacing:1px;margin-bottom:4px;">Stop Loss${pos.slSource ? " (" + pos.slSource + ")" : ""}</div>
    ...
    <div style="background:#0d1320;border:1px solid ${state._armedSignal ? '#7a5b16' : '#1a2236'};border-radius:12px;padding:20px 24px;text-align:center;">
      <div id="ajax-flat-banner" style="font-size:0.9rem;font-weight:600;color:${state._armedSignal ? '#f59e0b' : '#8ba1c2'};margin-bottom:14px;">${state._armedSignal ? `\ud83c\udfaf ARMED ${state._armedSignal.side} \u2014 waiting for next candle to cross ${state._armedSignal.triggerLevel} (${state._armedSignal.side === 'CE' ? 'above' : 'below'}) to enter` : 'FLAT \u2014 Waiting for entry signal'}</div>
...
<div class="top-bar">
  ...
  <div class="top-bar-right">
    ...
    ${state.running
      ? `<button onclick="secretGo('/bb_rsi-paper/stop', this)" style="background:#7f1d1d;border:1px solid #ef4444;color:#fca5a5;padding:5px 14px;border-radius:6px;font-size:0.72rem;font-weight:700;cursor:pointer;font-family:inherit;">Stop Session</button>`
      : `<button onclick="secretGo('/bb_rsi-paper/start', this)" style="background:#1e40af;border:1px solid #3b82f6;color:#fff;padding:5px 14px;border-radius:6px;font-size:0.72rem;font-weight:700;cursor:pointer;font-family:inherit;">Start BB_RSI Paper</button>`}
    ...
    <button onclick="spHandleReset(this)" style="background:#07111f;border:0.5px solid #0e1e36;color:var(--muted-1,#8ba1c2);padding:5px 11px;border-radius:6px;font-size:0.68rem;font-weight:600;cursor:pointer;font-family:inherit;">↺ Reset</button>
  </div>
</div>
...
<div class="stat-grid">
  ...
  <div class="sc" style="border-top:1.5px solid #6a5090;">
    ...
    <div class="sc-val"><span id="ajax-trade-count">${state.sessionTrades.length}</span> <span style="font-size:0.75rem;color:var(--muted-1,#8ba1c2);">/ ${_BB_RSI_MAX_TRADES}</span></div>
  ...
  <div class="sc" style="border-top:2px solid ${state._slPauseUntil && Date.now() < state._slPauseUntil ? '#ef4444' : '#4a6080'};">
    ...
    <div id="ajax-sl-pause-sub" style="font-size:0.7rem;margin-top:4px;color:var(--muted-1,#8ba1c2);">${_BB_RSI_PAUSE_CANDLES} candle cooldown after SL</div>
  </div>
  <div class="sc" style="border-top:2px solid ${state._dailyLossHit ? '#ef4444' : '#10b981'};">
    <div class="sc-label">Daily Loss Limit</div>
    <div class="sc-val" id="ajax-daily-loss-val" style="color:${state._dailyLossHit ? '#ef4444' : '#10b981'};">${state._dailyLossHit ? 'HIT' : 'OK'} <span style="font-size:0.65rem;color:var(--muted-1,#8ba1c2);">/ -\u20b9${_BB_RSI_MAX_LOSS.toLocaleString("en-IN")}</span></div>
    ...
    <div style="background:#0d1320;border:1px solid #1d3b6e;border-radius:16px;padding:24px 28px;max-width:720px;width:100%;max-height:90vh;overflow-y:auto;box-shadow:0 24px 80px rgba(0,0,0,0.9);position:relative;">
      <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:18px;">
        <div>
          ...
          <span style="font-size:0.65rem;color:var(--muted-1,#8ba1c2);margin-left:10px;">BB_RSI Paper \u2014 Full Details</span>
        </div>
...
(function() {
  ...
  async function fetchChart() {
    try {
      ...
      if (d.armedTrigger && !d.stopLoss && !selEt) { armedLine = cs.createPriceLine({ price:d.armedTrigger, color:'#f59e0b', lineWidth:2, lineStyle:LightweightCharts.LineStyle.Dashed, axisLabelVisible:true, title:'ARM ' + (d.armedSide||'') }); }
...
<div class="main-content">
  ...
  <p style="font-size:0.75rem;color:var(--muted-1,#8ba1c2);">Resolution: <strong>${BB_RSI_RES}-min</strong> candles</p>
