...
function _setHistoryFn(fn) { _historyFn = fn; }
function _resetHistoryFn()  { _historyFn = (params) => fyers.getHistory(params); }
...
async function mapLimit(items, limit, worker, onProgress, shouldStop) {
  ...
  async function run() {
    while (true) {
      ...
      catch (err) { results[i] = { ok: false, error: err && err.message ? err.message : String(err) }; }
      done++;
...
async function runScan(job, adapter, tf, symbols) {
  ...
  if (symbols.length >= 4 && failures.length >= Math.ceil(symbols.length * 0.6)) {
    ...
    systemic = { reason, count: n, total: symbols.length };
...
module.exports = {
  ...
  _setHistoryFn, _resetHistoryFn, CACHE_DIR, MAX_BARS,
};
