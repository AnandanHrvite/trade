...
## Log
- 2026-08-11: light-theme button colours on the Backtest, per-pattern Backtest and Live pages. `.run-btn` (dark navy), `.btn-copy` hover/copied (dark greens) and Stop/Exit (dark maroon/brown) had no entry in the light skin's hex rewriter, so they stayed dark-theme fills under text the skin had already darkened. All now paint as solid light-theme buttons (≥4.8:1). Styling only.
