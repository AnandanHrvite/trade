/**
 * BN_PIVOT_RSI_ST — NIFTY BANK: RSI extreme + Pivot R1/S1 breakout, SuperTrend-stopped
 * ═════════════════════════════════════════════════════════════════════════════
 * This is RSI_PIVOT_ST's rule set, unchanged, applied to a DIFFERENT UNDERLYING.
 * Every threshold, every stop and every omission below is identical to
 * src/strategies/rsi_pivot_st.js. Exactly three things differ, all of them
 * consequences of the instrument, not of the strategy:
 *
 *   1. SPOT      NSE:NIFTYBANK-INDEX, not NSE:NIFTY50-INDEX. Pivots, RSI and
 *                SuperTrend are all computed on NIFTY BANK candles.
 *   2. STRIKES   struck every 100 points, not 50 (BANKNIFTY_STRIKE_STEP).
 *   3. EXPIRY    MONTHLY ONLY. NSE withdrew BANKNIFTY weekly options in
 *                Nov-2024, so there is no weekly contract to buy — the option
 *                symbol is always the month code (e.g. NSE:BANKNIFTY26MAR54000CE).
 *                A monthly contract decays more slowly than a weekly, which
 *                makes the 25% premium floor a LOOSER stop in rupee terms than
 *                it is on NIFTY. That is a real behavioural difference and it
 *                is the instrument's doing, not a rule change.
 *
 * If a rule here ever needs to change, change it in BOTH engines or in neither
 * — the whole point of this strategy is that it is the same rules on NIFTY BANK.
 *
 * Single-leg BANKNIFTY option buying, intraday, 5-minute bars, Zerodha. Two levels
 ...
 *   CE:  RSI(14) > BN_PIVOT_RSI_ST_RSI_CE_MIN (70)  AND  the candle CROSSES AND
 ...
 * The percentage is BN_PIVOT_RSI_ST_STRIKE_PCT (default 1). The raw distance is
 * rounded to the nearest 100-point BANKNIFTY strike, and a 1% move that rounds
 * to 0 steps falls back to ATM rather than inventing a strike. 1% of a ~54,000
 * index is ~540 points, which rounds to 5 strikes — a real shift, unlike the
...
const NAME = "BN_PIVOT_RSI_ST";
const DESCRIPTION =
  "BN_PIVOT_RSI_ST — NIFTY BANK: RSI(14) extreme plus a 5-min candle crossing and closing beyond the previous day's " +
 ...
 * A typo in BN_PIVOT_RSI_ST_ENTRY_END must not silently collapse the entry window
...
/**
 * BANKNIFTY strikes are struck every 100 points. Read from the shared
 * underlying registry rather than hard-coded, so the one Settings field
 * (BANKNIFTY_STRIKE_STEP, under Common → NIFTY BANK) moves the strike grid for
 * this engine and for every future BANKNIFTY strategy at the same time. A
 * strike grid that disagrees between the engine and the symbol builder would
 * name a contract that does not exist.
 */
const UNDERLYING = "BANKNIFTY";
function strikeStep() {
  return instrumentConfig.underlyingOf(UNDERLYING).strikeStep;
}
/** Static default, exported for tests and for callers that want the nominal grid. */
 ...
 */
function getConfig() {
  const rawStrike = String(process.env.BN_PIVOT_RSI_ST_STRIKE_MODE || "OTM").trim().toUpperCase();
  const rawPremSides = String(process.env.BN_PIVOT_RSI_ST_PREMIUM_SL_SIDES || "BOTH").trim().toUpperCase();
  ...
  const rawStSides = String(process.env.BN_PIVOT_RSI_ST_ST_SIDES || "").trim().toUpperCase();
  const legacyCeOff = String(process.env.BN_PIVOT_RSI_ST_ST_CE_ENABLED || "true").toLowerCase() !== "true";
  const stSides = ST_SIDES.includes(rawStSides) ? rawStSides : (legacyCeOff ? "NONE" : "CE");
  return {
    resolutionMins:  _intEnv("BN_PIVOT_RSI_ST_RESOLUTION", 5, 1, 60),
    sessionStartMin: _parseHHMM(process.env.BN_PIVOT_RSI_ST_SESSION_START, 9 * 60 + 15),
    entryStartMin:   _parseHHMM(process.env.BN_PIVOT_RSI_ST_ENTRY_START,   9 * 60 + 30),
    entryEndMin:     _parseHHMM(process.env.BN_PIVOT_RSI_ST_ENTRY_END,    15 * 60),

    // The RSI half of the entry rule.
    rsiPeriod:       _intEnv("BN_PIVOT_RSI_ST_RSI_PERIOD", 14, 2, 100),
    rsiCeMin:        _numEnv("BN_PIVOT_RSI_ST_RSI_CE_MIN", 70, 0, 100),
    rsiPeMax:        _numEnv("BN_PIVOT_RSI_ST_RSI_PE_MAX", 40, 0, 100),
    ...
    strikePct:       _numEnv("BN_PIVOT_RSI_ST_STRIKE_PCT", 1, 0, 20),

    // SuperTrend — CE stop only. 10/2 is the user's stated setting.
    stPeriod:        _intEnv("BN_PIVOT_RSI_ST_ST_PERIOD", 10, 2, 100),
    stMultiplier:    _numEnv("BN_PIVOT_RSI_ST_ST_MULT", 2, 0.1, 10),
    // Which sides carry the SuperTrend stop: BOTH / CE (default, the original
    // rule) / PE / NONE. Legacy BN_PIVOT_RSI_ST_ST_CE_ENABLED=false still means
    ...
    premiumStopPct:  _numEnv("BN_PIVOT_RSI_ST_PREMIUM_SL_PCT", 25, 1, 90),
    ...
    maxDailyTrades:  _intEnv("BN_PIVOT_RSI_ST_MAX_TRADES", 5, 1, 50),
    maxDailyLoss:    _numEnv("BN_PIVOT_RSI_ST_MAX_DAILY_LOSS", 5000, 0),
    exitTime:        _parseHHMM(process.env.BN_PIVOT_RSI_ST_EXIT_TIME, 15 * 60 + 15),
  };
}
 ...
 *
 * OTM/ITM shift by `strikePct` of SPOT, rounded to the nearest BANKNIFTY strike
 * (100 points by default — see strikeStep()).
...
function strikeForSide(spot, side, cfg) {
  ...
  if (cfg.strikeMode === "ATM" || !(cfg.strikePct > 0)) {
    return { strike: atm, steps: 0, mode: "ATM", atm, step, distancePts: 0 };
  }

  const rawPts = spot * (cfg.strikePct / 100);
  const steps = Math.round(rawPts / step);
  if (steps <= 0) {
    return { strike: atm, steps: 0, mode: "ATM", atm, step, distancePts: 0 };
  }
  ...
  const strike = atm + dir * steps * step;
  return {
    strike,
    steps,
    mode: cfg.strikeMode,
 ...
 *
 * Which sides carry it is configurable (`BN_PIVOT_RSI_ST_PREMIUM_SL_SIDES`):
...
function getSignal(candles, opts) {
  ...
  if (!o.silent) {
    ...
    console.log(`[BN_PIVOT_RSI_ST ${ist}] ENTER ${side} @ ${entry} | ${side === "CE" ? `ST ${base.slSpot}` : "premium-stop only"} | strike ${base.strike} | RSI ${base.rsi}`);
  }
  return base;
}
