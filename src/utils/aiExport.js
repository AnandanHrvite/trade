...
const FIELD_LEGEND = {
  ...
  side: "The signal direction. On options: CE = call (bullish bet) · PE = put (bearish bet). On FUTURES (isFutures=true): CE = LONG · PE = SHORT, and a PE profits when NIFTY FALLS.",
  symbol: "Contract traded — an option symbol, or a NIFTY futures contract (…FUT) when isFutures=true.",
  qty: "Quantity in units (lot size × lots).",
  entryPrice: "Price paid at entry (₹): the option premium, or the NIFTY index level when isFutures=true.",
  exitPrice: "Price received at exit (₹): the option premium, or the NIFTY index level when isFutures=true.",
  spotAtEntry: "NIFTY spot index level when the trade was entered.",
  spotAtExit: "NIFTY spot index level when the trade was exited.",
  optionEntryLtp: "Option last-traded price at entry (₹). NULL on a futures trade — there is no premium; use entryPrice/spotAtEntry.",
  optionExitLtp: "Option last-traded price at exit (₹). NULL on a futures trade — use exitPrice/spotAtExit.",
  optionStrike: "Strike price of the option. NULL on a futures trade — futures have no strike.",
  optionType: "CE (call) or PE (put). NULL on a futures trade.",
  optionExpiry: "Option expiry date. NULL on a futures trade.",
  entryTime: "Entry timestamp, IST.",
  exitTime: "Exit timestamp, IST.",
  pnl: "Realised profit/loss for the trade in ₹ (negative = loss).",
  pnlMode: 'How the P&L was measured: "option premium" = from the option LTP move · "futures: entry ₹X → exit ₹Y (LONG|SHORT)" = index points × qty, direction-signed · "spot proxy" = estimated from the spot move when option data was unavailable.',
  ...
  instrument: "NIFTY_OPTIONS or NIFTY_FUTURES — what was actually traded.",
  isFutures: "true = a NIFTY futures position (CE=LONG, PE=SHORT, no premium/strike/expiry); false = a bought option.",
...
const PREFERRED_ORDER = [
  ...
  "spotAtEntry", "spotAtExit", "pnl", "pnlMode", "instrument", "entryReason", "exitReason",
];
