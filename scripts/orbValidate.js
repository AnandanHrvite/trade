...
(async () => {
  ...
  for (const d of DAYS) {
    ...
    for (let j = 0; j < dayC.length; j++) {
      ...
      const beTrig = Math.max(BE_PTS, BE_OR * (sig.rangePts || 0));
      for (const c of dayC.slice(j + 1).filter(c => MIN(c.time) <= EOD)) {
        ...
        if (side === "CE" ? c.low <= sl : c.high >= sl) { exit = sl; why = be ? "BE" : "SL"; break; }
        if (!be && fav >= beTrig) { sl = entry; be = true; }
        const e = emaAt(c.time, TRAIL);
        if (e != null) {
          if (!armed && (side === "CE" ? c.close > e : c.close < e)) armed = true;
          if (armed && (side === "CE" ? c.close < e : c.close > e)) { exit = c.close; why = "trail"; break; }
        }
      }
      if (exit == null) { const rest = dayC.slice(j + 1).filter(c => MIN(c.time) <= EOD); exit = (rest[rest.length - 1] || bar).close; why = "EOD"; }
